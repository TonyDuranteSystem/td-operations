...
export default function TaxFormCodePage() {
  const { token, code } = useParams<{ token: string; code: string }>()
  const searchParams = useSearchParams()

  const [isAdmin, setIsAdmin] = useState(false)
  const [submission, setSubmission] = useState<TaxFormSubmission | null>(null)
  ...
  const trackOpen = useCallback((sub: TaxFormSubmission) => {
  ...
  const loadSubmission = useCallback(async () => {
    try {
      ...
      const sub = resBody.data as TaxFormSubmission
      ...
      // Access code validated — auto-verify (skip email gate)
      setVerified(true)
  ...
  useEffect(() => {
    if (submission) {
      document.title = lang === 'en'
        ? `Tax Form — ${(submission.prefilled_data?.llc_name as string) || token}`
        : `Modulo Fiscale — ${(submission.prefilled_data?.llc_name as string) || token}`
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 2. Build submitted data (include arrays)
      const submittedData: Record<string, unknown> = { ...formData }
      if (members.length > 0) submittedData.additional_members = members
      if (transactions.length > 0) submittedData.related_party_transactions = transactions
  ...
  function renderField(field: FieldConfig) {
    if (field.isArray) return null // Arrays handled separately
    ...
    return (
      <div key={field.key} className={`tf-field ${changed ? 'tf-field-changed' : ''} ${prefilled ? 'tf-field-prefilled' : ''}`}>
        ...
        {field.type === 'boolean' ? (
          <div className="tf-bool-group">
            <button
              type="button"
              className={`tf-bool-btn ${value === true || value === 'true' ? 'tf-bool-active' : ''}`}
            ...
            <button
              type="button"
              className={`tf-bool-btn ${value === false || value === 'false' ? 'tf-bool-active' : ''}`}
  ...
  // ─── Render Arrays (Members / Transactions) ─────────────
        ...
        {members.map((member, i) => (
          <div key={i} className="tf-array-item">
            ...
            <div className="tf-array-fields">
              <input className="tf-input" placeholder={L.member_name} value={member.member_name || ''} onChange={e => { const m = [...members]; m[i] = { ...m[i], member_name: e.target.value }; setMembers(m) }} />
              ...
              <input className="tf-input" placeholder={L.member_itin_ssn} value={member.member_itin_ssn || ''} onChange={e => { const m = [...members]; m[i] = { ...m[i], member_itin_ssn: e.target.value }; setMembers(m) }} />
              <input className="tf-input" placeholder={L.member_tax_residency} value={member.member_tax_residency || ''} onChange={e => { const m = [...members]; m[i] = { ...m[i], member_tax_residency: e.target.value }; setMembers(m) }} />
              <input className="tf-input" placeholder={L.member_address} value={member.member_address || ''} onChange={e => { const m = [...members]; m[i] = { ...m[i], member_address: e.target.value }; setMembers(m) }} />
            </div>
          </div>
        ))}
      </div>
  ...
  function renderTransactions() {
    if (!submission || submission.entity_type !== 'SMLLC') return null
    return (
      <div className="tf-array-section">
        <div className="tf-array-header">
          <h3>{L.related_party_transactions}</h3>
          {TOOLTIPS.related_party_transactions && (
            <button type="button" className="tf-tooltip-btn" onClick={() => setActiveTooltip(activeTooltip === 'rpt_section' ? null : 'rpt_section')}>ℹ️</button>
          )}
          <button type="button" className="tf-add-btn" onClick={() => setTransactions(prev => [...prev, {}])}>
            {L.addTransaction}
          </button>
        </div>
        {activeTooltip === 'rpt_section' && TOOLTIPS.related_party_transactions && (
          <div className="tf-tooltip-box">{TOOLTIPS.related_party_transactions[lang]}</div>
        )}
        {transactions.map((txn, i) => (
          <div key={i} className="tf-array-item">
            <div className="tf-array-item-header">
              <span>#{i + 1}</span>
              <button type="button" className="tf-remove-btn" onClick={() => setTransactions(prev => prev.filter((_, j) => j !== i))}>
                {L.removeTransaction}
              </button>
            </div>
            <div className="tf-array-fields">
              <input className="tf-input" placeholder={L.rpt_company_name} value={txn.rpt_company_name || ''} onChange={e => { const t = [...transactions]; t[i] = { ...t[i], rpt_company_name: e.target.value }; setTransactions(t) }} />
  ...
  function renderStepContent(step: number) {
      ...
      return (
        <div className="tf-step-content">
          ...
          <div className="tf-docs-section">
            <h3>{L.documentsOnFile}</h3>
            <div className="tf-doc-item">
              <span>{L.articlesOfOrg}</span>
              {submission.has_articles_on_file ? (
                <span className="tf-doc-ok">✅ {L.onFile}</span>
              ) : (
                <div className="tf-doc-upload">
                  ...
                  <input
                    ...
                    onChange={e => setUploadFiles(prev => ({ ...prev, articles: e.target.files?.[0] || null }))}
                  />
                </div>
              )}
            </div>
            <div className="tf-doc-item">
              <span>{L.einLetter}</span>
              {submission.has_ein_letter_on_file ? (
                <span className="tf-doc-ok">✅ {L.onFile}</span>
              ) : (
                <div className="tf-doc-upload">
                  <span className="tf-doc-missing">📎 {L.uploadRequired}</span>
            ...
            {(submission.entity_type === 'MMLLC' || submission.entity_type === 'Corp') && (
              <div className="tf-doc-item">
                <span>{L.financialStatements}</span>
                <div className="tf-doc-upload">
                  <span className="tf-doc-optional">{L.uploadFinancials}</span>
                  <input
                    type="file"
    ...
    // Steps 1-3: render fields
    const stepTitle = step === 1 ? L.step1Title : step === 2 ? L.step2Title : L.step3Title
    ...
    return (
      <div className="tf-step-content">
        ...
        {step === 1 && renderMembers()}
        {step === 3 && renderTransactions()}
      </div>
    )
  }
  ...
  // Note: In code-based route, verification is automatic when access_code matches.
  // This gate is kept as fallback but should not normally be reached.
  ...
  const entityLabel = submission.entity_type === 'SMLLC' ? L.entitySMLLC : submission.entity_type === 'MMLLC' ? L.entityMMLLC : L.entityCorp
  const totalSteps = 4
    ...
    <>
      ...
      <div className="tf-container">
        ...
        <div className="tf-hero">
          <div className="tf-hero-label">{entityLabel}</div>
          <h1>{L.title}</h1>
          <p className="tf-hero-sub">{(submission.prefilled_data?.llc_name as string) || ''} — {submission.tax_year}</p>
        </div>
        ...
        <div className="tf-progress">
          {[1, 2, 3, 4].map(step => (
            ...
            >
              <div className="tf-progress-num">{step < currentStep ? '✓' : step}</div>
        ...
        <div className="tf-nav">
          {currentStep > 1 && (
            <button className="tf-nav-btn tf-nav-back" onClick={() => setCurrentStep(s => s - 1)}>
              ← {L.back}
            </button>
          )}
...
function TaxFormStyles() {
  return (
    <style jsx global>{`
      ...
      /* Boolean buttons */
      .tf-bool-group { display: flex; gap: 8px; }
      .tf-bool-btn { padding: 10px 24px; border: 2px solid var(--tf-gray-200); border-radius: 8px; background: #fff; font-size: 15px; font-weight: 500; cursor: pointer; transition: all .2s; color: var(--tf-gray-500); }
      .tf-bool-active { border-color: var(--tf-blue); background: var(--tf-blue-lighter); color: var(--tf-blue); font-weight: 600; }

      /* Arrays */
      .tf-array-section { margin-top: 24px; border-top: 1px solid var(--tf-gray-200); padding-top: 20px; }
      ...
      .tf-docs-section h3 { font-size: 16px; color: var(--tf-gray-700); margin-bottom: 16px; }
      ...
      .tf-doc-optional { color: var(--tf-gray-500); font-size: 13px; }
      ...
      @media (max-width: 640px) {
        ...
        .tf-bool-btn { padding: 10px 18px; font-size: 14px; }
      }
    `}</style>
  )
}
