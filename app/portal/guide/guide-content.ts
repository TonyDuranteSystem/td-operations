...
/**
 * The client portal's help-article library — plain data, no 'use client'
 * (dev job 12cab351). Split out of app/portal/guide/page.tsx so
 * lib/portal/guide-translatable-text.ts (imported by the server-side
 * translation job handler) can read the English content without pulling in
 * a client component — same separation wizard-configs.ts already has from
 * wizard-client.tsx.
 *
 * ARTICLES_EN/IT and GUIDE_CONTENT_EN/IT are the hand-written source of
 * truth for English and Italian (unchanged from before this split — still
 * used directly for those two locales). Any OTHER picked language is
 * resolved dynamically at render time in page.tsx via the same
 * "translations[english] ?? (locale === 'it' ? italian : english)" pattern
 * already used for wizard field labels — see guide-translatable-text.ts for
 * what actually gets fed to the AI translation engine.
 */
...
// The English template AI translation targets for the search-result count
// — kept as one non-pluralized phrase for any picked language outside
// English/Italian, which each keep their own hand-written pluralization
// below (resolveSearchResultCount in page.tsx picks between them).
export const RESULT_COUNT_TEMPLATE = '{n} results found'
...
export const ARTICLES_EN: Article[] = [
  ...
  },
  {
    ...
    steps: [
      { text: 'Click "Profile" in the left menu and scroll to the bottom of the page.' },
...
export const GUIDE_CONTENT_EN: Content = {
...
export const ARTICLES_IT: Article[] = [
  // ── Passo 1 — Configura il tuo Account ──
  {
    id: 'edit-profile',
  ...
  },
  {
    ...
    steps: [
      { text: 'Clicca "Profilo" nel menu a sinistra e scorri fino in fondo alla pagina.' },
...
export const GUIDE_CONTENT_IT: Content = {
