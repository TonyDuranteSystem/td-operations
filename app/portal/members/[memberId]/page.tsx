      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl border shadow-sm p-6">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-full bg-violet-100 text-violet-700 text-xl font-semibold flex items-center justify-center shrink-0">
              <Building2 className="h-6 w-6" />
            </div>
            <div className="flex-1">
              <h1 className="text-xl font-semibold text-zinc-900">{member.company_name}</h1>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="text-xs px-2 py-0.5 rounded-full bg-violet-100 text-violet-700">Company Member</span>
                {member.is_primary && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-600">Primary</span>
                )}
        ...
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-4 border-b bg-zinc-50">
            ...
            <span className="text-sm font-semibold text-zinc-800">Company Information</span>
          </div>
          <div className="divide-y">
            {member.ein && (
              <div className="flex items-center gap-3 px-5 py-3.5">
                ...
                <div>
                  <p className="text-[11px] text-zinc-400 uppercase tracking-wide font-medium mb-0.5">EIN</p>
                  <p className="text-sm text-zinc-700">{member.ein}</p>
                </div>
              </div>
            )}
            {companyAddress && (
              <div className="flex items-start gap-3 px-5 py-3.5">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-700">{companyAddress}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Representative */}
        {(member.representative_name || member.representative_email || member.representative_phone) && (
          <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b bg-zinc-50">
              <User className="h-4 w-4 text-zinc-500" />
              <span className="text-sm font-semibold text-zinc-800">Authorized Representative</span>
            </div>
            <div className="divide-y">
              {member.representative_name && (
                <div className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <div>
                    <p className="text-[11px] text-zinc-400 uppercase tracking-wide font-medium mb-0.5">Name</p>
                    <p className="text-sm text-zinc-700">{member.representative_name}</p>
                  </div>
                </div>
              )}
              {member.representative_email && (
                <div className="flex items-center gap-3 px-5 py-3.5">
                  <Mail className="h-4 w-4 text-zinc-400 shrink-0" />
                  <div>
                    <p className="text-[11px] text-zinc-400 uppercase tracking-wide font-medium mb-0.5">Email</p>
                    <a href={`mailto:${member.representative_email}`} className="text-sm text-blue-700 hover:underline">{member.representative_email}</a>
                  </div>
                </div>
              )}
              {member.representative_phone && (
                <div className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <div>
                    ...
                    <a href={`tel:${member.representative_phone}`} className="text-sm text-zinc-700 hover:text-zinc-900">{member.representative_phone}</a>
                  </div>
                </div>
              )}
            </div>
        ...
        {!member.ein && !companyAddress && !member.representative_name && !member.representative_email && (
          <div className="bg-white rounded-xl border shadow-sm p-10 text-center">
            <Shield className="h-10 w-10 text-zinc-300 mx-auto mb-3" />
            <p className="text-sm text-zinc-500">No additional details on file for this company member.</p>
          </div>
        )}
      </div>
    )
