...
export default function OnboardingFormCodePage() {
  ...
  const trackOpen = useCallback((sub: OnboardingSubmission) => {
  ...
  const loadSubmission = useCallback(async () => {
    try {
      ...
      const sub = resBody.data as OnboardingSubmission
  ...
  function isStepValid(step: number): boolean {
    if (step === 3 && submission?.entity_type === 'SMLLC') return true
  ...
  function getVisibleSteps(): number[] {
    ...
    if (submission.entity_type === 'SMLLC') return [1, 2, 4]
    return [1, 2, 3, 4]
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      try {
        await fetch('/api/onboarding-form-completed', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ submission_id: submission.id, token: submission.token }),
        })
      } catch { /* non-blocking — notification failure doesn't affect client */ }
        ...
        <div className="tf-step-content">
          ...
          <div className="tf-review-summary">
            ...
            <div className="tf-summary-grid">
              ...
              {formData.owner_itin ? <div><strong>{L.owner_itin}:</strong> {String(formData.owner_itin)}</div> : null}
              {formData.owner_itin_issue_date ? <div><strong>{L.owner_itin_issue_date}:</strong> {String(formData.owner_itin_issue_date)}</div> : null}
              <div><strong>{L.company_name}:</strong> {String(formData.company_name || '—')}</div>
