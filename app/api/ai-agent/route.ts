  ...
  if (attachments?.length) {
    try {
      ...
      if (sidebarSendable.length) {
        // READING is not EMAILING — the two halves of this block are gated
        // differently on purpose.
        //
        // The "read more of this file" note stays UNCONDITIONAL: reading the
        // rest of a long upload works on this screen whether or not email does,
        // and it is the fix for a spreadsheet past the first window being lost
        // entirely (td-bug, Luca).
        //
        // The ATTACHABLE list is deferred until the send rails are known and
        // only emitted where email is actually available — off a client page
        // this surface has no send_email tool at all, so advertising files to
        // attach promises a control that is not there.
        attachablePromptPending = true
