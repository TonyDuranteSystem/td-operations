    ...
    switch (action) {
      ...
      case "record_payment": {
        ...
        if (newPayment?.id && params.account_id) {
          ...
          //
          // This used to stamp `matched_payment_id` onto EVERY matched feed whose sender
          // name merely CONTAINED the company's name — no amount check, no limit, no
          // review. One diagnostic click could have attributed a dozen unrelated bank
          // transactions to a single payment.
          //
          // It never did, and only by accident: it wrote `match_confidence: "diagnostic"`,
          // a value the database's CHECK constraint rejects. The write failed every single
          // time, the code discarded the error, and nobody noticed. Verified against
          // production: ZERO rows have ever carried that value.
          //
          ...
          // other: through the Finance bank-feed screen, one transaction at a time, by a
          // human who can see the amount.
        }
