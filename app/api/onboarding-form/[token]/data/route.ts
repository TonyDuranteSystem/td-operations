/**
 * GET   /api/onboarding-form/[token]/data?code=...&preview=td   — fetch the onboarding submission
 * PATCH /api/onboarding-form/[token]/data                       — track-open, or record the completed submission
 *   body: { code, preview?, action: "track_open" | "submit", submitted_data?, changed_fields?, upload_paths? }
 *
 * Same pattern as app/api/tax-form/[token]/data/route.ts, adapted for
...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  const access = await verifyTokenAccess<OnboardingFormRow>(req, "onboarding_submissions", "*", token, code, preview)
  if (access.error) {
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const access = await verifyTokenAccess<OnboardingFormRow>(
    req,
    "onboarding_submissions",
