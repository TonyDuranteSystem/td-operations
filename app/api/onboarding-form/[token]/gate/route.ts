/**
 * GET  /api/onboarding-form/[token]/gate            — minimal info for the pre-code email gate
 * POST /api/onboarding-form/[token]/gate  { email }  — verify the typed email server-side, return the access code on match
 *
...
interface OnboardingFormGateRow {
  id: string
...
async function loadRow(token: string) {
  ...
  return data as unknown as OnboardingFormGateRow
}
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const key = `onboarding-form-gate:${clientIp(req) || "unknown"}:${token}`
