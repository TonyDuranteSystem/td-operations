    ...
    try {
      ...
      if (candidates?.length) {
        ...
        for (const c of candidates) {
          ...
          const noNMReport = (c.state_of_formation || "").toUpperCase().replace("NEW MEXICO", "NM") === "NM"
    ...
    if (created > 0 || blocked > 0 || watchdog.length > 0) {
      try {
        ...
        await gmailPost("/messages/send", {
          ...
          subject: `🔄 RA Renewal: ${created} new${blocked ? ` + ${blocked} blocked` : ""}${watchdog.length ? ` + ${watchdog.length} invisible` : ""}`,
          htmlBody: html,
        })
      } catch (emailErr) {
