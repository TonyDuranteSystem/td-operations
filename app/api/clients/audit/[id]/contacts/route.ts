...
export async function GET(req: NextRequest, { params: _params }: { params: { id: string } }) {
  ...
  const { data } = await supabaseAdmin
    .from('contacts')
    .select('id, full_name, email, phone, language, citizenship, itin_number, itin_issue_date, portal_tier, date_of_birth, passport_number, passport_expiry_date, passport_on_file, kyc_status, address_line1, address_city, address_state, address_zip, address_country')
