...
import { kickoffMissingTranslationWork } from '@/lib/portal/translation-generator'
  ...
  if (!(SUPPORTED_LOCALES as readonly string[]).includes(lang)) {
    try {
      ...
      if (brandNew && (await distinctLanguagesTranslatedToday()) >= MAX_NEW_LANGUAGES_PER_DAY) {
        ...
        // lags behind on its own separate track. Shared with the daily
        // top-up cron (dev job 4fa1d8e5) via lib/portal/translation-generator.ts.
        await kickoffMissingTranslationWork(lang, 'portal-language-picker')
      }
    } catch (e) {
