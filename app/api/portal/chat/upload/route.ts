// ⚠️ BACKWARD-COMPAT SHIM — DO NOT DELETE without a rollover window.
// Tabs that loaded BEFORE the 2026-05-30 chat-upload change still run the old
// client bundle, which POSTs the file here (multipart, streamed through the
// function — subject to the platform ~4.5MB body limit). Deleting this route
// made those open tabs hard-fail with a 404 ("Upload failed — please try
// again"). Freshly-loaded tabs use /api/portal/chat/upload-url (signed-URL
// direct-to-storage, 100MB) instead. Keep this route until old sessions have
// rolled over; it is harmless to leave in place.
