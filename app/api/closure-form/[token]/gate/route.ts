/**
 * GET  /api/closure-form/[token]/gate            — minimal info for the pre-code email gate
 * POST /api/closure-form/[token]/gate  { email }  — verify the typed email server-side, return the access code on match
 *
 * Same pattern as app/api/tax-form/[token]/gate/route.ts. The bare
 * closure-form page (app/closure-form/[token]/page.tsx) used to fetch the
 * FULL closure_submissions row with `select('*')` by token alone — no code
 * required at that stage — then compared the visitor's typed email against
 * `prefilled_data.owner_email` IN THE BROWSER. The entire row (including
 * access_code and owner_email) was already in the page's JS the moment it
 * loaded, before any email was typed: the "gate" was pure UI.
 */
...
async function loadRow(token: string) {
  ...
  return data as unknown as ClosureFormGateRow
}
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const key = `closure-form-gate:${clientIp(req) || "unknown"}:${token}`
