/**
 * GET  /api/banking-form/[token]/gate            — minimal info for the pre-code email gate
 * POST /api/banking-form/[token]/gate  { email }  — verify the typed email server-side, return the access code on match
 *
 * Same pattern as app/api/tax-form/[token]/gate/route.ts, adapted for
 * banking_submissions (prefilled_data.email, not .owner_email).
 */
...
async function loadRow(token: string) {
  const { data, error } = await supabaseAdmin
    ...
    .select("id, access_code, status, language, completed_at, prefilled_data, provider")
    .eq("token", token)
    .maybeSingle()
  if (error || !data) return null
  return data as unknown as BankingFormGateRow
}
...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  return NextResponse.json({
    ...
    hasOwnerEmail: !!(row.prefilled_data?.email as string | undefined),
    // Not sensitive (a provider name like "payset"/"relay") — needed so the
    // success screen renders the right provider's copy even when the full
    // row is never fetched (a completed submission doesn't need it).
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const key = `banking-form-gate:${clientIp(req) || "unknown"}:${token}`
