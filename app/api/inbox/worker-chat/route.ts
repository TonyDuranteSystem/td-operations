...
export async function POST(req: NextRequest) {
  ...
  // READABLE is a SUBSET of attachable. The reader resolves bytes from a path or
  // a stored URL, so a `document` entry — whose locator is a CRM row id, not a
  // location — cannot be read by it. Documents stay attachable; they are simply
  // not handed to the file reader, which would fail on them.
  const readableUploads = sendableFiles.filter(
    (f): f is typeof f & { source: "worker_upload" | "chat_asset" } => f.source !== "document",
  )
  ...
  try {
    const { reply, artifacts } = await callWorkerWithAttachments(userBody, {
      ...
      // READABLE is a SUBSET of attachable. The read pin resolves bytes from a
      // path or a stored URL, so a `document` entry — whose locator is a CRM row
