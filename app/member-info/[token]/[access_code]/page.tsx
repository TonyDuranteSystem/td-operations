...
interface MemberInfoRequest {
  ...
  pre_populated_data: { members: Partial<MemberRow>[] } | null
...
function MemberInfoContent() {
  ...
  const loadRequest = useCallback(async () => {
    try {
      ...
      // Pre-populate from existing members data
      if (req.pre_populated_data?.members?.length) {
        setMembers(req.pre_populated_data.members.map(fromPrePopulated))
      } else {
        setMembers([newMember('individual')])
      }
    ...
    <>
      ...
      <div className="mi-container">
        ...
        <div className="mi-hero">
          ...
          <p>Please provide the information for all members of the LLC. Each member&apos;s ownership percentage must be accurate, and the total must equal 100%.</p>
        </div>
...
function MemberCard({ member, index, total, onChange, onTypeChange, onRemove }: MemberCardProps) {
      ...
      <div className="mi-fields">
        {isIndividual ? (
          ...
          <>
            ...
            <div className="mi-section-label">Representative (Authorized Signer)</div>
...
function MemberInfoStyles() {
  return (
    <style jsx global>{`
      ...
      .mi-hero { text-align: center; padding: 24px 0 28px; }
      ...
      .mi-member-card { background: #fff; border-radius: 16px; box-shadow: 0 2px 12px rgba(0,0,0,.06); padding: 28px; }
      .mi-member-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; flex-wrap: wrap; gap: 12px; }
