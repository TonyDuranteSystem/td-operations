...
- **R070** — Run `git pull origin main` BEFORE any work, every session (enforced by SessionStart hook `session-git-pull.sh`). **Governs the PRIMARY CHECKOUT. As of 2026-08-10 (widened 2026-09-16) the hook deliberately does NOT stash or pull in ANY linked worktree, regardless of which branch it's currently on** — it silently removed in-flight work (4 occurrences on one job, one of them hiding a file an already-committed change depended on), and pulling main into a feature branch is a merge decision, not housekeeping. The exemption originally only applied when the worktree was on a named feature branch; a worktree left sitting on `main` right after a merge fell through to the same stash+pull and did it again on 2026-09-15, so the branch-name check was dropped — being a worktree at all is what matters. A worktree session that wants main's latest merges it deliberately. Full rationale: `docs/systems/hooks-guardrails.md` (worktree exemption).
