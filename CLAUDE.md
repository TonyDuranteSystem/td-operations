...
- **R111** — SLACK CODE-TASK RAIL IS ANTONIO-ONLY (2026-06-30, commit `69a0d04e`). The Slack worker's `start_code_task` / `promote_code_branch` tools (auto-launch/ship a Mac Mini coding session) are gated per-message on `ctx.slack_user_id === SLACK_USER_ANTONIO` in `processSlackEvent` (`lib/ai-agent/slack-claude.ts`) — checked on the CURRENT turn's sender, not thread-level, so a teammate (e.g. Luca) replying in a thread Antonio started still does not get the tool. Unresolved sender defaults to NOT-Antonio (safe default). Structural gate (the tool isn't offered to the model at all on a non-Antonio turn), same pattern as the in-channel approval-code rail — not just a prompt instruction. Origin: the worker autonomously launched a Mac Mini build + opened a PR off Luca's e-sign bug report with no instruction from Antonio. Antonio's words: *"I want the worker to rely only on me. When Antonio says investigate the issue or run a code, the worker will execute, nobody else."* Full detail in `docs/systems/slack-claude-worker.md` → "Code-task rail".
