...
4. **Environment verification — run BEFORE any implementation begins (R104):**
   - `cat .vercel/project.json` — confirm projectName. If `td-operations` → linked to PRODUCTION. If `td-operations-sandbox` → linked to sandbox.
   - `git branch --show-current` — confirm active branch. `main` → any push auto-deploys production.
   - `grep NEXT_PUBLIC_SUPABASE_URL .env.local` — confirm Supabase ref. `ydzipybqeebtpcvsbtvs` = production. `xjcxlmlpeywtwkhstjlw` = sandbox.
   - State the environment clearly: "Currently linked to: [production/sandbox]. Branch: [main/other]. DB: [production/sandbox]."
   - If any of the three point to production and the task requires code changes: **STOP and confirm with Antonio before writing a single line of code.**
   - Sandbox deploy + browser QA must complete before `git push origin main`. Override: `ALLOW_PRODUCTION_PUSH_AFTER_SANDBOX_QA=1 git push origin main` — only after Antonio's explicit approval in chat.
...
- **R103** — `portal_chat_send` MCP tool does NOT send email to the client — it writes directly to DB. The API route `/api/portal/chat` creates an in-app portal notification only (no email). Clients receive NO proactive notification unless you manually send a Gmail. Dev task `eabf304c` tracks building auto email notification for admin→client messages. Until that ships, always follow up a portal message with a Gmail if the client needs to act urgently.

- **R104** — ENVIRONMENT VERIFICATION MANDATORY before any implementation (2026-04-30). At session start AND before writing any code, run three checks: (1) `cat .vercel/project.json` — if `projectName` is `td-operations` you are linked to PRODUCTION; (2) `git branch --show-current` — if `main`, any push auto-deploys production; (3) `grep NEXT_PUBLIC_SUPABASE_URL .env.local` — confirm sandbox ref `xjcxlmlpeywtwkhstjlw` vs production `ydzipybqeebtpcvsbtvs`. If any of the three points to production and the task requires code changes, STOP and confirm with Antonio. Sandbox deploy + Antonio browser QA must complete before pushing to main. Override: `ALLOW_PRODUCTION_PUSH_AFTER_SANDBOX_QA=1 git push origin main` — only after explicit Antonio approval. Do NOT use `--no-verify`. The pre-push hook enforces this: pushing to main while `.vercel/project.json` points to production is blocked by default. Root cause of 2026-04-29 Phase 0 incident: `.vercel/project.json` was linked to production; push to main bypassed sandbox-first.
