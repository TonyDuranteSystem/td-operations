...
- **R097** — QB sync is MANUAL ONLY (2026-04-21, commit `1dbfa33`). QuickBooks is a one-way DOWNSTREAM destination, not a source of truth. NO automatic QB sync triggers exist anywhere — Stripe webhook, Whop webhook, activate-service workflow, and the dedicated `qb-invoice-sync` cron all had their QB calls REMOVED (cron deleted entirely). The MCP tool `qb_create_invoice` and `lib/quickbooks.ts::createInvoice` REQUIRE the caller to pass our canonical `invoice_number` (INV-NNNNNN format from `lib/portal/invoice-number.ts`) — QB no longer mints. Antonio runs sync manually when he wants (typically ~once a year before the tax return). The dedicated "Push to QuickBooks" button in the CRM finance page is the only intended trigger (deferred build, not urgent — see open-cases item 3). Do NOT add automatic QB sync to any new code; do NOT recommend re-enabling background QB calls.
