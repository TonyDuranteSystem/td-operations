  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        ...
        {value && (
          <FastTooltip label="Edit this registry entry">
            <button
              type="button"
              onClick={openEdit}
              disabled={!selectedRow || saving || fetchLoading}
              aria-label="Edit this registry entry"
        ...
        <FastTooltip label="Add new registered agent entry">
          <button
            type="button"
            onClick={openAdd}
            disabled={saving || pickSaving}
            aria-label="Add new registered agent entry"
      ...
      {selectedRow && (
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs pl-0.5 mt-0.5">
          <span className="text-zinc-500">
            ...
            {selectedRow.agent_name ?? <span className="italic text-zinc-300">—</span>}
          </span>
          <span className="text-zinc-500">
            <span className="text-zinc-400">Provider:</span>{' '}
            {selectedRow.provider ?? <span className="italic text-zinc-300">—</span>}
          </span>
          {selectedRow.county ? (
