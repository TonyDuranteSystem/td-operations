...
const STAGE_COLORS: Record<string, { badge: string; icon: string }> = {
  'Initial Consultation': { badge: 'bg-zinc-100 text-zinc-700', icon: 'text-zinc-500' },
  'Offer Sent': { badge: 'bg-amber-100 text-amber-700', icon: 'text-amber-500' },
  'Negotiation': { badge: 'bg-blue-100 text-blue-700', icon: 'text-blue-500' },
  'Agreement Signed': { badge: 'bg-indigo-100 text-indigo-700', icon: 'text-indigo-500' },
  'Closed Won': { badge: 'bg-emerald-100 text-emerald-700', icon: 'text-emerald-500' },
}
  ...
  return (
    <div className="bg-white rounded-lg border p-3 text-sm hover:shadow-sm transition-shadow">
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        <div className="flex items-center gap-1.5">
          {deal.service_type && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600 truncate max-w-[100px]">
              {deal.service_type}
            </span>
          )}
          {deal.payment_status && (
            <span className={cn('text-xs px-1.5 py-0.5 rounded', PAYMENT_COLORS[deal.payment_status] ?? 'bg-zinc-100')}>
              {deal.payment_status}
            </span>
          )}
        </div>
...
function KanbanView({ stages }: { stages: StageGroup[] }) {
  // Filter out Closed Won for kanban (it's usually too big)
  const activeStages = stages.filter(s => s.stage !== 'Closed Won')

  return (
    <div className="flex gap-4 overflow-x-auto pb-4">
      {activeStages.map(group => {
        const colors = STAGE_COLORS[group.stage] ?? STAGE_COLORS['Initial Consultation']
        return (
