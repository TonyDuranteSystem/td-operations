...
const TAX_DOCUMENTS_BASE: FieldConfig[] = [
  {
    ...
    repeaterFields: [
      ...
      // Identity build (2026-08-13, card 4a39e0fd): this field IS the account
      // identity for account_number-mode institutions. Statically optional —
      // requiredness is MODE-CONDITIONAL in getStepErrors (a static required
      // would demand a number from Wise/Revolut clients, the opposite of the
      // ruling) + server-enforced in wizard-submit's tax path.
      { name: 'account_label', label: 'Account number — or its last 4 digits (required for banks)', labelIt: 'Numero di conto — o le ultime 4 cifre (obbligatorio per le banche)', type: 'text', required: false, hint: 'This is what keeps two accounts at the same bank apart in your books.', hintIt: 'È ciò che distingue due conti della stessa banca nei tuoi libri contabili.' },
  ...
  financials: [
    ...
    ], hint: 'This is one of the most important questions for your tax filing. "Related party" means ANY company connected to you or your family — including companies you own in other countries, companies that paid your LLC, or companies your LLC paid. If your LLC received money from or sent money to ANY company connected to you, the answer is YES.', hintIt: 'Questa è una delle domande più importanti per la tua dichiarazione. "Parte correlata" significa QUALSIASI società collegata a te o alla tua famiglia — incluse società che possiedi in altri paesi, società che hanno pagato la tua LLC, o società a cui la tua LLC ha pagato. Se la tua LLC ha ricevuto o inviato denaro a QUALSIASI società collegata a te, la risposta è SÌ.', warningOnValue: { value: 'No', text: 'You are declaring that your LLC had NO transactions with any company owned by you, your family members, or any entity connected to you. If this is incorrect, the IRS may apply a $25,000 penalty. Please review carefully before proceeding.', textIt: 'Stai dichiarando che la tua LLC NON ha avuto transazioni con nessuna società di tua proprietà, dei tuoi familiari, o collegata a te. Se questo non è corretto, l\'IRS può applicare una sanzione di $25.000. Verifica attentamente prima di procedere.' } },
    {
      ...
      repeaterFields: [
        ...
        { name: 'rpt_amount', label: 'Transaction Amount ($)', labelIt: 'Importo Transazione ($)', type: 'number', min: 0, required: true, hint: 'Total USD value of transactions with this party for the year. Use the exchange rate at the time of the transaction.', hintIt: 'Valore totale in USD delle transazioni con questa parte per l\'anno.' },
    ...
    // Phase 3R (2026-07-04): broadens the related-party section — companies
    // owned/controlled even with NO transactions. Names feed categorization
    // (lib/tax/declared-entities.ts): transfers to these entities are FLAGGED
    // related-party so they can never silently read as vendor expenses.
    { name: 'other_owned_companies', label: 'Do you (or the members) own or control any other companies — even with no transactions with this LLC? List their names.', labelIt: 'Tu (o i soci) possedete o controllate altre società — anche senza transazioni con questa LLC? Elenca i nomi.', type: 'textarea', required: false, placeholder: 'e.g. LOREA LLP, Rossi Consulting SRL — one per line', placeholderIt: 'es. LOREA LLP, Rossi Consulting SRL — una per riga', hint: 'Knowing your other companies lets us recognize transfers between them and this LLC automatically, so they are never mistaken for normal business expenses or income.', hintIt: 'Conoscere le tue altre società ci permette di riconoscere automaticamente i trasferimenti tra loro e questa LLC, così non vengono mai scambiati per spese o ricavi normali.' },
...
export const TAX_MMLLC_FIELDS: Record<string, FieldConfig[]> = {
  ...
  compliance: [
    ...
    // Phase 3R (2026-07-04): same declared-entities question as the SMLLC
    // financials — names feed related-party flagging in categorization.
    { name: 'other_owned_companies', label: 'Do you (or the members) own or control any other companies — even with no transactions with this LLC? List their names.', labelIt: 'Tu (o i soci) possedete o controllate altre società — anche senza transazioni con questa LLC? Elenca i nomi.', type: 'textarea', required: false, placeholder: 'e.g. LOREA LLP, Rossi Consulting SRL — one per line', placeholderIt: 'es. LOREA LLP, Rossi Consulting SRL — una per riga', hint: 'Knowing your other companies lets us recognize transfers between them and this LLC automatically, so they are never mistaken for normal business expenses or income.', hintIt: 'Conoscere le tue altre società ci permette di riconoscere automaticamente i trasferimenti tra loro e questa LLC, così non vengono mai scambiati per spese o ricavi normali.' },
...
// ─── TD COMMUNICATION (Brand Audit) — minimal code-side fallback ───
// The REAL brand-audit questions live in the DB (td_comm_questions), rendered via
// buildTdCommWizardConfig (lib/td-communication/question-to-field.ts) and passed
// to WizardClient as `configOverride` from app/portal/wizard/page.tsx. This tiny
// config is ONLY the safety net used when the DB returns no active questions
// (fresh env / not-yet-seeded / load error), so the wizard still renders
// something valid instead of crashing. Both client_type paths use it identically.
// Keys chosen to match the DB set (brand_name / business_description /
// additional_notes) so the staff brief groups them correctly (pipeline.ts).
export const TD_COMM_FALLBACK_STEPS: WizardStep[] = [
  { id: 'brand', title: 'Brand Audit', titleIt: 'Brand Audit', description: 'Tell us about your brand', descriptionIt: 'Raccontaci del tuo brand' },
]

export const TD_COMM_FALLBACK_FIELDS: Record<string, FieldConfig[]> = {
  brand: [
    { name: 'brand_name', label: 'Business / brand name', labelIt: 'Nome dell\'attività o del brand', type: 'text', required: true },
    { name: 'business_description', label: 'Tell us about your business. What do you do?', labelIt: 'Raccontaci della tua attività. Di cosa ti occupi?', type: 'textarea', required: true, aiAssist: true },
    { name: 'additional_notes', label: 'Anything else we should know', labelIt: 'Altro che dovremmo sapere', type: 'textarea', required: false },
    { name: 'disclaimer_accepted', label: 'I confirm this information is accurate', labelIt: 'Confermo che queste informazioni sono corrette', type: 'checkbox', required: true },
  ],
}
...
export function getWizardConfig(wizardType: string, entityType?: string, bankingProvider?: string) {
  ...
  switch (wizardType) {
    ...
    case 'td_communication':
      // The real questions come from the DB and are passed to WizardClient as
      // `configOverride` (built in app/portal/wizard/page.tsx). This returns only
      // the minimal fallback used when the DB has no active questions.
      return { steps: TD_COMM_FALLBACK_STEPS, fields: TD_COMM_FALLBACK_FIELDS }
    default:
      return {
