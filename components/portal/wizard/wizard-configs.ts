...
export const TAX_MMLLC_FIELDS: Record<string, FieldConfig[]> = {
  ...
  compliance: [
    { name: 'comp_foreign_accounts', label: 'Does the company have an account at a bank in ANOTHER country — for example a local bank in your home country, opened in the company\'s name?', labelIt: 'La società ha un conto presso una banca di un ALTRO paese — per esempio una banca locale del tuo paese, intestato alla società?', type: 'select', required: true, options: YN, hint: 'The accounts we helped you open — Mercury, Relay, Wise, Revolut, Slash — are US accounts and do NOT count, even when they hold euros or other currencies. Answer Yes ONLY if the company has an account at a non-US bank, for example an Italian, Portuguese or Emirati bank. Most clients answer No.', hintIt: 'I conti aperti tramite noi — Mercury, Relay, Wise, Revolut, Slash — sono conti USA e NON contano, anche quando contengono euro o altre valute. Rispondi Sì SOLO se la società ha un conto presso una banca non americana, per esempio una banca italiana, portoghese o emiratina. La maggior parte dei clienti risponde No.' },
