...
export function BankAccounts({ accountId }: { accountId: string }) {
  ...
  const handleSetInvoice = async (id: string) => {
    try {
      ...
      toast.success('Invoice account updated')
            ...
            <div key={acc.id} className={`border rounded-xl p-4 transition-colors ${acc.show_on_invoice ? 'border-blue-300 bg-blue-50/50' : ''}`}>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    {acc.show_on_invoice
                      ? <><CheckCircle2 className="h-3.5 w-3.5" /> On Invoice</>
                      : <><Circle className="h-3.5 w-3.5" /> Use on Invoice</>
                    }
                  </button>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-zinc-600">
                {acc.account_holder && <div><span className="text-zinc-400">Holder:</span> {acc.account_holder}</div>}
                {acc.bank_name && <div><span className="text-zinc-400">Bank:</span> {acc.bank_name}</div>}
                {acc.iban && <div><span className="text-zinc-400">IBAN:</span> {acc.iban}</div>}
                {acc.swift_bic && <div><span className="text-zinc-400">SWIFT:</span> {acc.swift_bic}</div>}
                {acc.account_number && <div><span className="text-zinc-400">Account #:</span> {acc.account_number}</div>}
                {acc.routing_number && <div><span className="text-zinc-400">Routing #:</span> {acc.routing_number}</div>}
                {acc.notes && <div className="col-span-2 mt-1 text-zinc-400 italic">{acc.notes}</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      {showAdd ? (
        <div className="border rounded-xl p-4 space-y-3 bg-zinc-50">
          <div className="flex items-center gap-3 mb-1">
            <span className="text-sm font-medium text-zinc-700">New Bank Account</span>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setCurrency('EUR')}
              ...
              <Euro className="h-4 w-4" /> EUR Account
            </button>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-zinc-700">Show on invoices</span>
          </label>

          <div className="flex gap-2 pt-1">
            <button onClick={handleAdd} disabled={saving}
              className="flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Add Account
            </button>
