    ...
    <div className="space-y-4">
      ...
      {showAdd ? (
        <div className="border rounded-lg p-4 space-y-3 bg-zinc-50">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-zinc-600 mb-1">Gateway</label>
          ...
          <div>
            <label className="block text-xs text-zinc-600 mb-1">Payment URL *</label>
            <input type="url" value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="https://buy.stripe.com/..."
              className="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-zinc-600 mb-1">Amount (optional)</label>
          ...
          <div className="flex gap-2">
            <button onClick={handleAdd} disabled={saving}
              ...
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Add Link
            </button>
            <button onClick={() => setShowAdd(false)} className="px-4 py-2 text-sm border rounded-lg hover:bg-zinc-50">Cancel</button>
          </div>
        </div>
      ) : (
      ...
      {links.length === 0 && !showAdd && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          ...
          <p className="text-xs text-blue-700 mb-3">Low fees, instant payouts, and simple setup.</p>
          <a href="https://whop.com/tony-durante-llc?a=myllcexpert" target="_blank" rel="noopener noreferrer"
            ...
            <ExternalLink className="h-3.5 w-3.5" /> Get Started with Whop
          </a>
        </div>
      )}

      <p className="text-xs text-zinc-400">
        Payment links appear as "Pay Now" buttons on your invoices. The default link is used automatically unless you choose a specific one.
      </p>
      <p className="text-xs text-zinc-400 italic">
        Stripe Connect integration coming soon — connect your Stripe account to create payment links automatically from the portal.
      </p>
    </div>
  )
}
