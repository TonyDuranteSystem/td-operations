...
function fromPrePopulated(data: Partial<MemberRow>): MemberRow {
  ...
  // null DB values must become empty strings — spreading nulls breaks controlled inputs
...
/**
 * Member info form — handles individual + company members, ownership %,
 * SS-4 signer selection, and submission.
 *
 * Used by the public route (app/member-info/...) and the portal viewer
 * (app/portal/form/...). Token-authenticated; no Supabase session required.
 *
 * @param embedded  — when true, drops full-page backgrounds, logo, and
 *                    height:100vh states so the form sits cleanly inside
 *                    the portal layout.
 * @param adminMode — when true, shows the amber ADMIN PREVIEW badge and
...
export function MemberInfoForm({
  ...
  if (error) {
    if (embedded) {
      return (
        <>
          <MemberInfoStyles embedded />
          <div className="p-8 max-w-md mx-auto mt-4 bg-white rounded-lg border text-center">
            <p className="text-red-600">Link not found or invalid. Please contact your advisor.</p>
          </div>
        </>
      )
    }
      ...
      return (
        <>
          ...
          <div className="p-8 max-w-md mx-auto mt-4 bg-white rounded-lg border text-center">
            ...
            <h1 className="text-xl font-semibold text-gray-900 mb-2">Information Submitted</h1>
            <p className="text-gray-600">
              Thank you! We have received the member information
              {request?.company_name ? ` for ${request.company_name}` : ''}.
              We will process it and update your account shortly.
            </p>
          </div>
        </>
      )
  ...
  return (
    <>
      <MemberInfoStyles embedded={embedded} />
      <div className="mi-container">
        {/* Header — logo only shown in non-embedded (public) mode */}
        {!embedded && (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            disabled={submitting || adminMode || members.length === 0}
          >
      ...
      <div className="mi-member-header">
        ...
        <div className="mi-member-actions">
          <div className="mi-type-toggle">
            <button type="button" className={`mi-type-btn ${isIndividual ? 'mi-type-active' : ''}`} onClick={() => onTypeChange(member.id, 'individual')}>Individual</button>
            <button type="button" className={`mi-type-btn ${!isIndividual ? 'mi-type-active' : ''}`} onClick={() => onTypeChange(member.id, 'company')}>Company</button>
          </div>
          {total > 1 && (
            <button type="button" className="mi-remove-btn" onClick={() => onRemove(member.id)}>Remove</button>
          )}
        </div>
      </div>
...
/**
 * Injects mi-* class styles globally. In embedded mode, the body-level
 * overrides (background, font-family, color) and the Google Fonts import
 * are suppressed so the portal layout's own styles are not affected.
 * All mi-* class definitions are always included — they only apply when
 * elements with those classes are in the DOM.
 */
function MemberInfoStyles({ embedded = false }: { embedded?: boolean }) {
  return (
    <style jsx global>{`
      ${!embedded ? `
