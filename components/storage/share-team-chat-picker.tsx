...
/**
 * Team Chat destination picker for CRM Storage's "Share" action. Same shape
 * as components/captures/team-chat-destination-picker.tsx (reuses the same
 * GET /api/team/threads listing + label helper), adapted to a storage
 * fileId instead of a captureId and with no resend concept — see
 * lib/crm-storage/share-actions.ts for why.
 */
import { useEffect, useMemo, useState } from 'react'
import { Loader2, MessageSquare, Search } from 'lucide-react'
import { sendStorageFileToTeamChat } from '@/lib/crm-storage/share-actions'
...
export function ShareTeamChatPicker({
  fileId,
  onSent,
  onError,
}: {
  ...
  const sendTo = async (threadId: string) => {
    setBusy(true)
    try {
      await sendStorageFileToTeamChat(fileId, threadId)
      onSent()
    } catch (err) {
      setBusy(false)
