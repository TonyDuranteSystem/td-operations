...
_Latest 2026-06-14 — Claude (Client-facing flow surface added on the PORTAL side: `lib/flows/flow-progress.ts::computeFlowProgress` (pure, unit-tested) computes a client progress fraction + current `client_label` from a flow's `pipeline_stages` and the SD's stage. Consumed by `getPortalFlows()` (`lib/portal/queries.ts`) for the portal dashboard "Service Status" card; the portal documents page groups `documents.service_delivery_id`-stamped docs into per-flow sections. Staff workspace unchanged — read-only client view over the same SDs. Full writeup in `portal.md`.)_
