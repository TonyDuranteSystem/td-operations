...
_Last verified against code: 2026-06-14 — Claude (Clickable stepper: `stage-stepper.tsx` is now a client component — clicking any stage (other than the current) moves the SD directly to it, forward OR backward, after a "Move to [stage]?" confirm, via `POST /api/flows/[id]/set-stage` → `setServiceDeliveryStage` (lib/operations/service-delivery.ts). That helper is a LIGHTWEIGHT manual position-correction: sets stage/stage_order/stage_entered_at + appends stage_history + keeps status coherent (terminal target → completed+end_date; else active+cleared end_date). It does NOT create auto-tasks, notify the client, delete documents, or bump renewal dates — those stay with the action buttons (`/advance`) and Go Back (`/revert`). The "← Go Back" button visibility was verified already-correct: shown on every stage except the first.)_
...
- `app/api/flows/[id]/` — `advance` (target_stage, full side effects), `set-stage` (direct jump to any stage — stepper clicks, no side effects), `revert` (go back one stage), `upload-document` (upload + optional auto-advance via `auto_advance` body flag), `documents` (list), `submission` (latest tax submission), `send-for-signature` (create signature request + notify + advance), `signature` (latest request status), `chat` (per-SD client-facing `portal_messages` stream: GET oldest-first + POST staff send auto-stamping `service_delivery_id` + `topic` (flow name) + `account_id`/`contact_id` + client notification).
