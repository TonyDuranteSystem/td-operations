# Tax Returns & Filings
_Last verified against code: 2026-06-10 — Claude (Bug fix: portal tax submissions copied 0 attachments to Drive + produced no P&L/Balance Sheet — root cause was a storage-bucket mismatch (portal uploads to `onboarding-uploads`, but `saveFormToDrive("tax_return")` defaulted to the external form's `tax-form-uploads`). Fix: added `opts.bucket` override to `saveFormToDrive`; portal handler passes `onboarding-uploads`, `tax_form_review` infers bucket from path prefix. See new gotchas. Affected: MPG Performance / Dynamiq SR / Nexo Agency 2025.)_
