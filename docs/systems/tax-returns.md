...
- **Read-only in Slice 6** (cards link to `/accounts/[id]`). Drag-drop, bulk actions, assignees, payment indicators = Slice 7; payment-aware auto-advance + board↔What's New sync = Slice 8.
...
- **The bank-statement parser accepts only PDF and CSV** (`lib/bank-statement-parser.ts`) — `.zip` archives of monthly statements are NOT parsed (they're copied to Drive as-is for the accountant, but yield no transactions and no P&L). Clients who upload zipped statements need the archive extracted to PDF/CSV before `bank_statement_process` can build the P&L.
