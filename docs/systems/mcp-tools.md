# MCP Tool Server
_Last verified against code: 2026-06-04 — Claude (WP1: agent approvals group += hermes_heartbeat / approval_claim / approval_complete, and approval_decide(approve) now requires a confirmation_code — see the Agent approvals group bullet + agent-bridge.md WP1)_
...
- **Agent approvals group (2026-06-04, Phase 2 Slice 1 + Slice 2; WP1):** `registerAgentApprovalTools` (`lib/mcp/tools/agent-approvals.ts`) over the `approval_queue` table — now **5 tools**: `approval_list` (read-only) **and** `approval_decide(id, 'approve'|'reject', confirmation_code?, note?)` **and** the WP1 Operating-Agent pull rail `hermes_heartbeat(instance_id)` / `approval_claim(instance_id)` / `approval_complete(id, status, result?, error_text?)`. `approve` flips pending→approved and fires the `app/api/cron/approval-executor` worker, which runs the REAL action (kill-switch-gated by `APPROVAL_RAIL_ENABLED`); `reject` flips pending→rejected. **WP1: `approve` now REQUIRES the proposal's 6-digit `confirmation_code` — a missing/wrong code leaves the row pending and runs nothing.** `approval_claim` atomically claims the oldest approved+unclaimed row for an external instance to execute, `approval_complete` finalizes it; both coexist with the server executor (atomic claim prevents double-exec). `approval_decide` carries the same mandatory show-the-draft-and-wait-for-OK rule as `gmail_send`. Full subsystem doc: `agent-bridge.md` (WP1).
- **Agent threads group (2026-06-04, Phase C):** `registerAgentThreadTools` (`lib/mcp/tools/agent-threads.ts`) adds `thread_search(query, type?, tags?, limit?)` (read-only) over the `thread_summaries` table — searches the bridge's durable thread memory so Hermes can reference a past investigation instead of re-deriving it. Full subsystem doc: `agent-bridge.md` (Phase C).
