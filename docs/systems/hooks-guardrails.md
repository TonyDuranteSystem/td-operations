...
_Last verified against code: 2026-06-29 — Claude (`stop-check.sh`: added a `/clear` context tip that fires when recent commits exist AND working tree is clean — task-boundary detection; emits a 💡 recommendation only, never automated.)_
...
- **Worktree stack CLEANUP** — two layers. (1) Immediate: when a worktree's job ships (PR merged) the session OFFERS to tear it down (`env-down.sh --purge`) per **R110** — never auto-runs without confirmation. (2) Backstop: `scripts/worktree-stack-sweep.sh` runs backgrounded from the auto-isolate SessionStart hook on EVERY session (even the main checkout) and purges any stack whose worktree folder is gone. Orphan detection is clone-agnostic: `env-up.sh` records the absolute worktree path in `~/.td-local-stacks/<name>/.worktree-path`, and the sweep purges only when that path no longer exists (a stack with no recorded path is left alone — safe default). This is why closed worktrees never leak their ~8 GB stacks.
