...
_Last verified against code: 2026-08-26 — Claude (full catalog audit, see whats-new.md's matching entry for the full writeup. The relevant finding for THIS board: `itin_renewal_upcoming` is a real `ActEvent`, used by the January-only ITIN renewal cron (`app/api/cron/deadline-reminders/route.ts`), with a correctly-written seed migration (`scripts/migrations/20260614-1201-itin-renewal-action-event.sql`) that had never actually been promoted to production — so `emitActionNeeded()` would have silently created NO card at all the next time that cron ran. Promoted to production now (using sandbox's actual current wording, which had drifted from the migration file's original text — sandbox's copy was created 2026-07-17, presumably a direct catalog edit, and reads "Time to renew the ITIN — sell Renewal + 1040-NR" rather than the file's original "Initiate ITIN renewal — file Form W-7 with 1040-NR"). Not a proven missed real-world case — the cron didn't get a real chance to run against this catalog row before now, since the feature shipped in June, after January 2026 had already passed — but would have failed on its next scheduled run (January 2027) if left unfixed. Worth updating the migration file itself to match what's actually live, as a follow-up.)_
_Prior: 2026-08-26 — Claude (dev job 9b7892d6: **client confirmation of P&L/Balance Sheet now creates a card.** Added `financials_confirmed` to `ActEvent` + a matching `action_events` catalog row (migration `20260826-1500-financials-confirmed-action-event.sql`, scope `account`, default assignee Luca). Call site: `app/api/portal/tax-financials/attest/route.ts`, placed in the route's own awaited path right after the confirmation write succeeds — not inside the pre-existing fire-and-forget archive handoff (`lib/tax/attest-handoff.ts`), which is exposed to the same class of silent-skip risk the banking fix closed the day before. That handoff's own plain `tasks` insert for this same event was REMOVED in the same change (it had become a second, duplicate staff signal once this card existed) — the outcome-specific detail it used to carry (archived / no Drive folder / archive failed) now goes to `action_log` instead of a task. Also: the portal Company Closure wizard (dev job fbbf4abe) now reliably creates its own SD-lifecycle card via the pre-existing `closure_progress` workflow + `createSD`, converged with the older emailed-link closure pipeline onto one implementation (`lib/jobs/handlers/closure-setup.ts`) — no new `ActEvent` needed there, since `dispatchWorkflowForSdCreated` already handled it; the gap was purely that the portal wizard path never reached `createSD` at all.)_
