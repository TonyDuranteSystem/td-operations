...
_Last verified against code: 2026-09-19b — Claude (**NEW: REAL TELEGRAM CHANNEL (RECEIVE + REPLY), dev job da2cf91b.** Antonio, after the WhatsApp-badge/Telegram-mislabeling fix (2026-09-19 below) surfaced that Telegram had unread real messages with no UI anywhere to see them — one conversation, "Daniel X accountant", had 4 genuine unread inbound messages from 2026-07-28 sitting invisible for ~7 weeks — asked for the "real feature": Telegram working the same way WhatsApp does, not just a read-only viewer. **Investigated first, found real send/receive infrastructure did NOT exist**: `messaging_channels` had a stale `platform='telegram'` row from 2026-03-08 with a `webhook_secret` already set but no webhook route anywhere in the code, and `lib/messaging/send-dispatcher.ts`'s `PROVIDER_HANDLERS` never had a `telegram_bot_api` entry despite the DB constraint already allowing that provider value — meaning Telegram could receive nothing live and send nothing at all. The 3 existing `messaging_groups` under that channel read as leftover test/prototype data (one has `-100123456789`, Telegram's own API-docs placeholder chat id). **Also found and confirmed as a real, separate risk before building anything:** the only `TELEGRAM_BOT_TOKEN` already in this codebase belongs to Hermes (`lib/ai-agent/telegram-notify.ts`) — Antonio's own personal research/notification bridge — reusing it for client messaging would put real client conversations through his personal approval bot. Antonio created a fresh, dedicated bot (`@Tony_Durante_bot`) via BotFather specifically to keep these separate; its token is stored ONLY as the Vercel env var `TELEGRAM_CLIENT_BOT_TOKEN` (sandbox: `td-operations-sandbox` project, Production environment) — never committed, never re-displayed after the one message it arrived in.

**Built, mirroring the existing 2Chat/WhatsApp architecture exactly where it fit, deliberately NOT sharing components where the underlying capabilities differ:**
- `lib/messaging/telegram-groups.ts::findOrCreateTelegramGroup` — Telegram's counterpart to `groups.ts::findOrCreateWhatsAppGroup`, but with NO identifier normalization: a Telegram `chat.id` is already a stable, provider-issued key, used verbatim as `external_group_id` (WhatsApp's JID transformation doesn't apply and isn't needed).
- `lib/messaging/telegram-dispatcher.ts::dispatchTelegramMessage` — calls the Telegram Bot API's `sendMessage` directly (Telegram has exactly one provider, so no `PROVIDER_HANDLERS`-style registry the way WhatsApp needs one across 2Chat/Meta/Twilio); records the outbound copy into `messages` on success, same best-effort-save tradeoff as `dispatchWhatsAppMessage`.
- `app/api/webhooks/telegram/[channelId]/route.ts` — the inbound receiver. **Auth deliberately differs from the 2Chat webhook's `?secret=` URL param**: Telegram's `setWebhook` natively supports a `secret_token` that it echoes back as the `X-Telegram-Bot-Api-Secret-Token` header on every call — used instead, since it never puts the secret in a URL (server logs, proxies) the way a query param would. Still fail-closed: a channel with no `webhook_secret` accepts nothing. Only plain-text `message` updates are stored in this pass; `edited_message`/`channel_post`/stickers/etc. are acknowledged (200, so Telegram doesn't retry) but not persisted — matches the "receive + reply" scope this was built for, same as WhatsApp's own media handling was a later, separate addition.
- `app/api/inbox/reply/route.ts`'s Telegram branch **was calling a Supabase Edge Function that no longer exists in this repo** — the exact same defect WhatsApp had here until 2026-09-17 (see that entry below) — now dispatches through `dispatchTelegramMessage` for real.
- `app/api/inbox/telegram/{conversations,mark-read,messages/[groupId]}` — near-identical copies of the WhatsApp routes of the same name (the underlying logic is genuinely platform-agnostic — `messaging_groups`/`messages` don't care which platform wrote them); kept as separate per-channel files rather than generalized, matching this codebase's existing convention of per-channel route files over a shared generic one.
- `app/api/inbox/stats/route.ts` refactored: the per-platform unread-count logic (added in the 2026-09-19 fix below) is now one `unreadCountForPlatform()` helper reused for both `whatsapp` and `telegram`, so a third channel can't repeat the original mislabeling mistake. `InboxStats` gained a `telegram` field.
- UI: `InboxChannel` type gained `'telegram'`; a Telegram tab (sky-blue `Send` icon) with its own unread badge sits next to WhatsApp in the Inbox header; `components/inbox/telegram-thread.tsx` is a **deliberately slim, TEXT-ONLY sibling of `whatsapp-thread.tsx`, not a shared/generalized component** — WhatsApp's thread view is wired to WhatsApp-specific attachment staging and an AI-suggest endpoint that don't exist for Telegram, and building those just to satisfy one shared component would be scope neither asked for nor verified as needed; it reuses the same open-time mark-read pattern (`trackOpenMarkRead`) that fixed WhatsApp's own unread-badge bug, built in from the start here instead of retrofitted later. Several `!isWhatsApp`-gated "Gmail-only" UI elements in `inbox-shell.tsx`/`conversation-list.tsx` (the mailbox selector, the Gmail search+filter bar, the bulk-action bar, bulk-select checkboxes, Gmail's own pager) would have incorrectly applied to Telegram rows too since Telegram isn't WhatsApp — each was re-gated to also exclude `isTelegram`. The CRM quick-create actions (Create Task/Service/Invoice) and the AI worker button were LEFT AS `!isWhatsApp`-gated deliberately — a pre-existing code comment already read "stay Gmail/Telegram-only until WhatsApp has its own reviewed assist flow" (dev job f331cd43), confirming Telegram was already meant to keep those.
- **Known limitation, not fixed here — flagged for Antonio**: Telegram bots have a platform "Privacy Mode" that, when a bot is added to a GROUP chat, restricts it to only seeing messages that @-mention it or reply to it, not every message — confirmed via `getMe` showing `can_read_all_group_messages: false`. 1:1 direct messages to the bot are unaffected. A group like "Daniel X accountant" may need Privacy Mode turned off (BotFather → `/setprivacy`) or the bot made a group admin to see everything, once that specific conversation is revived.
- **Also NOT done in this pass, separate explicit-approval decision**: cleaning up the 3 stale production `messaging_groups` rows under the old 2026-03-08 channel. Only `Test Gruppo TG` (the `-100123456789` placeholder id) is unambiguously fake; `Daniel X accountant` must be preserved (it has Antonio's real unanswered messages) and `Midnight Pearl` is undecided — none deleted without Antonio's explicit go per row.

New tests: `tests/unit/telegram-groups.test.ts`, `tests/unit/telegram-dispatcher.test.ts`, `tests/unit/webhooks-telegram.test.ts` (auth via the header not a query param, non-text/non-message updates acknowledged-not-stored, inbound ingestion, sender-name fallback chain, redelivery dedup), `tests/unit/inbox-stats.test.ts` extended for the new `unreadCountForPlatform` shape. Full suite green (827 files, 11,297 tests), typecheck and lint clean, full `next build` clean (new routes `/api/inbox/telegram/*` and `/api/webhooks/telegram/[channelId]` confirmed present in the build output). **Verified live so far**: the bot token confirmed valid directly against Telegram's own `getMe` API (`username: Tony_Durante_bot`); a fresh `webhook_secret` generated and stored on the sandbox channel row; the token stored as a Vercel env var on the sandbox project only, never committed. **NOT yet verified**: an actual end-to-end message round-trip (Telegram → webhook → CRM → reply → Telegram) — this needs `setWebhook` registered against the deployed sandbox URL and a real chat to test against, which requires Antonio to message the bot from his own Telegram first (a bot cannot message a chat that hasn't messaged it first — Telegram's own platform rule, not a gap in this build). Production promotion (a webhook registered against the production URL, the same env var set on the production Vercel project, and the stale-row cleanup decision) is a separate, later, explicit-approval step.
