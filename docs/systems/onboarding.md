# Onboarding
_2026-08-07 — Claude (**Two portal-app install placements added at the onboarding moment** — Phase 4 of the install-adoption project, sandbox. (1) The portal welcome CHAT message posted by the onboarding-setup job gained one install-tip line with a link tagged `?src=onboarding`. **Copy-only:** the job's duplicate guard matches the `Welcome to Tony Durante LLC! We%` PREFIX, which the appended line does not touch, and no enqueue logic changed (the LT runaway-loop guards are untouched). (2) The welcome-package EMAIL template (`generateWelcomeEmail` in `lib/mcp/tools/welcome-package.ts`) gained a bilingual "your company's private area, on your phone" section with the same tagged link, placed before the closing — that email is already reviewed per-send by Antonio, so it needed no new gate. Rationale: onboarding is peak engagement, the highest-converting moment to ask for the install. Full subsystem detail in `pwa.md` → Phase 4.)_
