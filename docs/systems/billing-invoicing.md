# Billing & Invoicing
_Last verified against code: 2026-06-18 — Claude (invoice recipient resolution centralized in `resolvePaymentRecipient()`; owner-role match is now CASE-INSENSITIVE and all three send paths — CRM resend route, sendTDInvoice, sendPaidReceipt — delegate to it. Fixes the ADWise lowercase-"owner" resend failure, dev_task `63431fc8`.)_
...
- `lib/portal/resolve-payment-recipient.ts` — `resolvePaymentRecipient()`: the SINGLE source of truth for "who does an invoice email go to". Used by the CRM resend route (`app/api/invoices/[id]/send/route.ts`), `sendTDInvoice`, `sendPaidReceipt`, the pay-link route, and create-invoice-checkout. Unit-tested.
