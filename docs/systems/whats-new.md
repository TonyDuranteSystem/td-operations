...
_Last verified against code: 2026-08-30 — Claude (dev job `10995181` follow-up: added `card_autopay_enabled` — fires when a client enrolls a card for autopay (`saveAutopayCard()` in `lib/operations/card-autopay.ts`, the real Stripe webhook completion path). Followed the retire-and-re-emit pattern from day one rather than discovering the gap later: `retireCardAutopayEnabledNote` fires unconditionally from `disableAutopayCard()` (both the client's own self-service disable and the new staff-facing CRM toggle — see billing-invoicing.md) so a disable-then-re-enroll cycle notifies staff again instead of deduping forever against the first note, same shape as `retireBankingWizardSubmittedNote`. All three touchpoints this doc's own Gotchas section warns about were added in the same change: `EVENT_KEY_LABELS` ("Autopay") + a new `deepLinkFor()` case for source table `'accounts'` (routing to `/accounts/<id>`, the first event kind to need this table — the CRM Overview tab is where the corresponding Finance card lives) in `thread-whats-new-panel.tsx`, plus a `whats_new_events` catalog seed row (`scripts/migrations/20260830-2200-seed-whats-new-card-autopay-event.sql`, sandbox only — this event is NOT relying on the no-row-means-visible default) and a `WHATS_NEW_DEFAULT_STEPS` entry ("No action needed..."). Live-verified in sandbox: the note fires with the correct company name + last4, the panel shows the "AUTOPAY" badge, and the "Open" button correctly lands on the account page.)_
