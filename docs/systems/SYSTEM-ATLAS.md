...
- **E-Sign (internal e-signature engine)** — In-house DocuSign-class engine: visual field placement, multi-signer, templates, server flatten + Certificate of Completion. Multi-tenant schema (`owner_account_id`/`origin`); TD-first live, client product is Phase 5. **Sandbox only — not yet on production.**
