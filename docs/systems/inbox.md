...
_Last verified against code: 2026-07-22 — Claude (**No Inbox query refetches on window focus, and that is deliberate and enforced.** Context: the CRM now catches up when Antonio brings his phone PWA back to the front. The obvious way to build that — flip `refetchOnWindowFocus` ON globally and opt the expensive screens OUT — was tried, reviewed, and PULLED before shipping: the first sweep of Gmail-backed queries found one of five. The direction is the problem. Forgetting to opt a cheap query IN is invisible and harmless; forgetting to opt a Gmail query OUT is a quota incident. This surface has already drawn blood — the default INBOX list still issues ~300 live Gmail metadata GETs per load, and a bulk action once starved the per-user quota and blanked the list (which is why `components/inbox/inbox-shell.tsx` debounces the Gmail-push invalidation by 2.5s). So the global default stays OFF and `lib/query/refresh-on-focus.ts` is an explicit ALLOW-LIST; nothing in the Inbox is on it. **Six keys are recorded there as excluded-for-cost and MUST stay excluded:** `inbox-conversations`, `inbox-messages`, `inbox-stats`, `gmail-labels`, `client-emails` (N+1 — one Gmail call per thread), `email-unread`. Liveness here is unchanged: Gmail push plus each screen's own poll. **A unit test enforces this** (`tests/unit/refresh-on-focus-allowlist.test.ts`): it resolves every allow-listed key to its route file and fails if that route's GET calls Gmail or Drive — verified to actually bite by poisoning the list. **Check the GET handler specifically, not the file:** several routes call Gmail in POST (sending) while their GET is pure DB — `/api/portal/chat` is exactly that, and a file-level grep wrongly marks it expensive. Revisit only when the conversation list reads from `email_index` instead of live Gmail; at that point the quota argument disappears.)_
_Prior: 2026-07-21c — Claude (**SECURITY: the email-templates route had NO auth on ANY verb.** `app/api/inbox/templates/route.ts` read, created, edited and hard-DELETED templates with the service-role client and no identity check. It was reachable by any logged-in user — including a portal CLIENT — because middleware's `isDashboardPath()` returns false for everything under `/api`, so an API route inherits only "is logged in", never "is staff". All four verbs now call the new canonical `requireStaffRoute()` (`lib/auth/require-staff-route.ts`, deny-by-default, fails CLOSED if the identity check throws, 5 unit tests incl. "a portal client is refused"). Every caller is a staff surface (`app/(dashboard)/email-templates/page.tsx`, `components/inbox/compose-dialog.tsx`) — nothing under `app/portal` touches it — so the guard cannot affect a client using the portal. Found by an audit for the unfiltered-read shape behind the To-Do card leak; the same pass also gated `agent-decisions` and `addresses`. **Invariant: any new staff API route must call `requireStaffRoute()` as its first line — middleware will NOT do it for you.**)_
