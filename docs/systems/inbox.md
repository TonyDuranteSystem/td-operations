...
_Last verified against code: 2026-07-22 — Claude (**The conversation list now explicitly OPTS OUT of refetch-on-window-focus, and that opt-out is load-bearing.** Context: `components/providers.tsx` had `refetchOnWindowFocus: false` set GLOBALLY, which was why the CRM never caught up when Antonio foregrounded his phone PWA. That default is now `true` — but this list must not follow it. The default INBOX view still issues **~300 live Gmail metadata GETs per load** (see the known-debt note below), so refetching it on every tab-back would multiply that by every focus event, and a bulk action has ALREADY starved the per-user Gmail quota once — the 2.5s trailing debounce on the Gmail-push subscription in `components/inbox/inbox-shell.tsx` exists because of exactly that incident. Liveness here is unchanged and still comes from the push subscription plus the 75s poll in `components/inbox/conversation-list.tsx`. **RULE for anything new on this surface:** an event may invalidate a counts/index-backed query, never the live-Gmail list. Revisit the opt-out only once the list reads from `email_index` instead of live Gmail — at that point the quota argument disappears and it should follow the global default. Any future query added here that is similarly expensive must opt out LOCALLY rather than flipping the global back off.)_
