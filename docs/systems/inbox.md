...
_Last verified against code: 2026-07-20b — Claude (**Two corrections to the same-day change below, both found in council review; full story in `flows.md` 2026-07-20c.** **(1) DOUBLE TASK:** routing "create service from email" through `createSD` means `createSD` now creates the tracked task itself (the workflow task, or its universal fallback when the service type has no workflow). The route's own raw task insert — harmless when it followed a raw SD insert — would have given Luca TWO near-identical tasks for every service created from an email. That insert is removed; `createSD` owns it. **(2) FRIENDLY ERRORS:** three of the twelve service types in this dialog (Public Notary, Shipping, Support) have NO configured delivery pipeline in production — verified by live query. The old raw insert hid that by stamping a fake stage; `createSD` correctly refuses, but the refusal was leaking the internal error text to staff. The route now translates the three realistic failures into plain English: no pipeline configured for that service type, this is a personal service that needs a contact linked to the account, or this client already has an active service of that type. NOT on production — Antonio ships.**)_
_Last verified against code: 2026-07-20 — Claude (**"Create service from email" now goes through `createSD` instead of a raw insert, and its dialog stops swallowing the server's reason.** Found during the duplicate-ITIN investigation (full story in `flows.md` 2026-07-20b; dev job `d6b9ee70`). `app/api/inbox/create-from-email/route.ts` (`type:'service'`) raw-inserted into `service_deliveries` with a literal `stage:'New'` / `stage_order:0` — **not a real pipeline stage** — and left `contact_id` NULL. For ITIN that broke the Phase 1 contact-scoped rule: the SD was account-scoped, therefore INVISIBLE to both the per-person ITIN duplicate guard and the new `uq_itin_sd_active_per_contact` index, so this door could still mint a duplicate ITIN. It now calls `createSD`, which resolves the real first stage from `pipeline_stages` and auto-resolves the contact from the account (and forces ITIN contact-scoped). On failure the route returns the real reason, with a plain-English message for the duplicate case ("This client already has an active X service — open it instead of creating a second one"). **R099:** `components/inbox/create-from-email-dialog.tsx` was the textbook antipattern — `if (!res.ok) throw new Error('Failed to create service')` + `catch { toast.error(...) }` — so that message would have been thrown away; both the task and service branches now parse the error body and the catch surfaces `err.message`. Behaviour change worth knowing: services created from an email now start at their pipeline's real first stage rather than the fake `'New'`, and creating one for a service the client already has is refused with an explanation instead of silently duplicating. The two raw `tasks` inserts in this route are unchanged (no generic `createTask` helper exists yet — deferred, dev_task `fda76fd3`). NOT on production at time of writing — Antonio ships.**)_
