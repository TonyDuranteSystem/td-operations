...
_Last verified against code: 2026-07-21 — Claude (**PARKED WIP — "create service from email" routed through `createSD`. NOT SHIPPED, do not merge without finishing the open item below.** Branch `claude/inbox-create-service-hardening`, dev job `1fda3961`. Origin: the duplicate-ITIN investigation. The route raw-inserted into `service_deliveries` with a literal `stage:'New'` (not a real pipeline stage) and left `contact_id` NULL — so an ITIN created here was ACCOUNT-scoped, violating the Phase 1 contact-scoped rule and therefore invisible to BOTH the per-person ITIN guard and `uq_itin_sd_active_per_contact` (shipped 2026-07-21). This door could still mint a duplicate. **This branch:** calls `createSD` (real first stage, contact auto-resolved from the account, ITIN forced contact-scoped); DROPS the route's own task insert because `createSD` already creates one (workflow task, else universal fallback) and staff would otherwise get TWO per service; and fixes the dialog's R099 antipattern (`throw new Error('Failed to create service')` + `catch { toast.error(...) }`) so the server's real reason reaches the user, with plain-English text for the three realistic failures. **OPEN REGRESSION blocking the merge:** the dialog's `SERVICE_TYPES` lists 12 options but production `pipeline_stages` has NO rows for **Public Notary / Shipping / Support** (verified by live query — 17 types, none of those three), so `createSD` throws and those three become DEAD CONTROLS that fail only after the dialog is filled in. Previously the raw insert succeeded with a bogus stage. Fix by seeding those pipelines, filtering the dropdown to types that have one, or disabling the option with an explanation up front. **Also open:** the fallback task title doubles the service type ("ITIN — ITIN — Acme LLC"); and the screen has NEVER been opened in a browser (R079).**)_
