...
- **New-document alerts (admin → client)** (`lib/portal/document-alerts.ts`, 2026-06-06): when staff make a document client-visible, the client is alerted, the Documents sidebar tab **pulses** with an unopened count, and the doc shows a **"New" pill + indigo-tinted row** until that person opens it. Detection is app-level — `notifyClientsOfNewDocument(documentId)` is called fire-and-forget from the admin upload routes (`app/api/crm/admin-actions/upload-document`, `upload-account-document`). It's idempotent via `documents.client_notified_at` (TOCTOU-guarded), respects the per-doc `documents.notify_client` toggle and the global kill switch `app_settings.new_document_alert_enabled` (default true), and creates a `type='new_document'` portal_notification + web push. **Delivery rule mirrors chat:** PWA clients get the push; non-push clients get the batched digest email — the `portal-digest` cron skips the email for `new_document` when the recipient has a push subscription (no double-notify). **"New" is per-CONTACT** via `portal_document_views(document_id, contact_id, viewed_at)` so each owner of a multi-member LLC clears their own state; opening a doc records the view (`POST /api/portal/documents/[id]/view` + the download proxy as backstop). **Baseline:** pre-feature docs keep `client_notified_at = NULL` and never show as "new" (no day-one flood; no backfill). Sidebar count via `getUnopenedDocsCount`; the tab stops pulsing once the client is on `/portal/documents`.
