...
Last verified against code: 2026-08-22c — Claude (**Closed a real hole in the unauthenticated exception (below) the same day it shipped — an adversarial bug-hunter review caught it before production, not after.** The exception's own safety claim ("deduped by route+message, never floods the table") was false under adversarial input: `message` is 100% caller-controlled free text, `computeErrorFingerprint` only strips UUIDs/hex/emails/numbers from it, and the exact allowlisted route string is trivially readable by anyone (it has to be public — it ships inside `public/portal-sw.js`, which the browser must be able to fetch). Varying the message per request produced a genuinely distinct fingerprint every time, so dedup never engaged — an anonymous caller could flood the row list one request every few seconds (safely under the rate limit) and permanently bury the one real signal this exception exists to surface, with no cleanup job anywhere to age old rows out. **Fix:** `canonicalizeUnauthMessage()` in the route file collapses the message down to one of four fixed strings — unsupported / rejected / threw-synchronously / unrecognized — BEFORE it can influence the fingerprint, for unauthenticated requests only (authenticated callers are untouched, verified by test). An attacker now gets at most four rows on this route, ever, no matter how many requests they send. The exact original text is not thrown away — it still lands in a `raw_message` field inside `context`, which a human reads directly; only the DEDUP KEY stops trusting caller-supplied free text. Two residual risks reviewed and knowingly accepted, not fixed: the rate limiter is in-memory per server instance (Vercel can spin up several under load, so the true ceiling could be a multiple of the stated 20/minute — pre-existing limitation of the shared helper, shared by every other caller of it); and the IP address the rate limiter reads is spoofable if the trust assumption about Vercel's own edge network ever fails (same assumption a legally-relevant signing flow elsewhere in the codebase already relies on). Worst case on both, per the review: dashboard noise, never money or client data. Tests: `tests/unit/system-errors-report-route.test.ts`, now including the exact attack reproduced and closed.)

Prior: 2026-08-22b — Claude (**The badge-diagnostic caller (below) is exempted from the login requirement — the FIRST unauthenticated route this endpoint has ever accepted, dev job `61f62c08`.** Proven live, not assumed: the badge report's outbound request was confirmed reaching this endpoint (TLS handshake + valid certificate for our own domain, watched via the device's own network log) yet nothing ever landed in `system_errors` — the service worker's background push context does not reliably carry a login session, so the request was very likely getting the pre-existing 401 with nothing to show for it. Antonio's explicit call: build a version that does not depend on being logged in at all. **How it stays narrow, not a general "report anything" door:** a fixed allowlist (currently one entry, `portal-sw:push:setAppBadge`) is checked BEFORE the login check — only a request whose route exactly matches skips authentication; every other route is completely untouched, still requires login, still 401s exactly as before (regression-tested). An unauthenticated report is additionally rate-limited per IP (20/minute, the shared `lib/portal/rate-limit.ts` helper already used elsewhere) since nothing here identifies the caller, and still passes through the same size-capping every path already goes through. Captured with no email on the row (there is no session to attribute it to). Tests: `tests/unit/system-errors-report-route.test.ts`.)
