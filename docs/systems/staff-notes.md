...
_Last verified against code: 2026-07-23 — Claude (**DRAGGABLE BUTTONS are LIVE. PER-PERSON DONE/SNOOZE is BUILT AND SANDBOX-VERIFIED BUT NOT YET ON PRODUCTION — it needs migration `20260723-1000-per-person-note-state-and-thread-later.sql` run first, which Antonio does by hand (execute_sql refuses production DDL by design). Ship the code only AFTER that migration, or every note feed 500s on a missing table.**
