...
- **Whole-page capture silently fails on the Dashboard specifically** — found live during Phase 2
  QA, works cleanly on every other page tried (Notes, etc.). Not investigated further.
  **Correction, 2026-09-23:** this entry previously said a dev-tracker follow-up job had been
  filed for this ("see dev-tracker, 'Fix whole-page screenshot capture failing on the
  Dashboard'") — a live query of the real tracker during the region-capture investigation below
  found no such job was ever actually created. Still genuinely unfixed and unroot-caused; file a
  real job before starting on it, don't assume one already exists.

### Region capture of an Inbox email rendering blank (2026-09-23)
- **html2canvas's own cross-frame clone-and-paint is unreliable for this app's sandboxed email
  iframe — confirmed live, not assumed.** `EmailHtmlFrame` renders untrusted email HTML inside a
  `sandbox="allow-same-origin allow-popups allow-popups-to-escape-sandbox"` `srcDoc` iframe (no
  `allow-scripts`, security audit H8/H9). When `captureRegion`'s normal `html2canvas(document.body,
  ...)` walks into that iframe as part of the outer page, the result is correct for SOME emails'
  HTML and a near-blank render for others — confirmed with a real A/B on two real emails using the
  identical component, no error surfaced either way. Root cause not pinned to one exact CSS/markup
  trigger inside html2canvas's own internals; ruled out via direct live testing: NOT an
  image-load timing race (the email's images were already `complete: true` before the outer
  render even started), NOT a CORS/auth failure on the same-origin attachment proxy (200
  responses, images loaded fine in html2canvas's own clone too, confirmed via its `onclone`
  hook), NOT fixed by toggling html2canvas's `foreignObjectRendering` option (tested both ways,
  still broken). What IS confirmed reliable: calling html2canvas DIRECTLY on the iframe's own
  `iframe.contentDocument.body` — never letting html2canvas's cross-frame walk touch it at all —
  always rendered correctly in every test.
- **A genuinely separate bug, found only because the first attempted fix was tested live and
  failed: html2canvas's own returned canvas silently refuses to be drawn ONTO.** Compositing the
  separately-rendered iframe canvas via `ctx.drawImage(iframeCanvas, ...)` straight onto the
  canvas html2canvas itself returned produced ZERO visible change — no exception, no error,
  `getImageData` afterward showed the exact same pixels as before the draw call. Confirmed by a
  direct live A/B with otherwise identical parameters: the same `drawImage` call onto a canvas
  created locally (`document.createElement('canvas')`) worked correctly and produced the full,
  correct composited image. html2canvas's returned canvas CAN still be READ from (`getImageData`
  works) and drawn FROM as a source (`ctx.drawImage(html2canvasCanvas, 0, 0)` onto something else
  works) — it specifically refuses to be a destination. Root cause not pinned inside html2canvas's
  own internals; the robust fix is procedural, not a workaround for one specific cause: **never
  mutate a canvas a third-party library handed you — copy it onto a canvas you created yourself
  first, then draw anything else onto that.** `captureRegion` does exactly this now
  (`lib/captures/render.ts`). Worth remembering as a general pattern for any FUTURE code that
  post-processes an html2canvas result, not just this one.
- **The compositing math is split into pure, DOM-free functions specifically so it's unit-testable**
  (`lib/captures/email-frame-composite.ts`, `tests/unit/capture-email-frame-composite.test.ts`) —
  intersection/overlap math, the identity+geometry "did anything change" check, and the
  region-vs-whole-page coordinate placement are all plain functions taking plain rects, with the
  actual DOM querying (`findOverlappingEmailFrameSnapshots`, `readEmailFrameCurrentState`) kept as
  thin, deliberately-untested wrappers around them (this repo's vitest config runs in a `node`
  environment, no DOM).
- **Detection uses a dedicated `data-capture-email-frame` attribute, never the iframe's `title`**
  (`"Email content"`) — `title` is decorative accessibility copy with no contract; a future
  wording/i18n change to it must never be able to silently regress this fix back to the blank-
  image bug with zero compile-time signal.
- **Identity for the "did the captured email change mid-capture" check reuses the ALREADY-EXISTING
  `data-mid` marker** on the message card (`components/inbox/message-thread.tsx`) — not a new
  marker invented for this fix. A capture is dropped (falls through to the "may be missing
  content" notice, never guessed at) if the iframe was removed from the DOM entirely (collapsed,
  navigated away from, the whole page changed) OR its identity changed OR its exact position/size
  changed even slightly (a late-loading image growing the email taller between when the region was
  selected and when compositing actually happens) — checked once, immediately before compositing,
  not continuously.
- **A proposed UI "lock" during rendering (to close a race where the captured email's conversation
  could be switched mid-capture) was considered and REJECTED** after two rounds of review —
  Antonio's explicit request, "challenge it twice." It would have required new coupling between
  this generic capture tool and Inbox-specific internals with no prior art anywhere in this
  codebase, and no clean release guarantee on every exit path (closing the tool mid-render, a
  thrown error) — this exact bug class, state not reset when the tool closes mid-flight, is this
  feature's single most repeated real defect (see the "Closing the tool must invalidate..." and
  "isOpen/isBrowseOpen mutually exclusive" entries below). The identity+geometry re-check above is
  the actual guard; the lock would only have reduced how often that check has to drop something,
  at a real cost in new risk, for a race window that's milliseconds wide in practice (both render
  passes run back-to-back with no network round trip in either).
- **A possible security hole — html2canvas's OWN internal cross-frame cloning re-executing script
  content the iframe's sandbox blocks — was raised, then tested live with an actual payload, then
  closed.** Built a test iframe with identical sandbox attributes to the real component, containing
  both an inline `onerror` handler and a raw `<script>` tag; confirmed neither fired in the
  original iframe (baseline), and confirmed neither fired after running html2canvas directly
  against that same iframe's own `contentDocument.body` (the exact operation this fix performs).
  Independently re-confirmed by reading html2canvas's own nested-iframe source: it extracts the
  live DOM tree via `iframe.contentWindow.document.documentElement` and paints it through its own
  `CanvasRenderer` — it never re-serializes the HTML into a new document/iframe/`srcDoc`, so there
  is no path for inert (sandbox-blocked) script content to ever execute this way.
- **`captureWholePage` deliberately does NOT get this fix** — it has the separate, still-open,
  not-yet-root-caused Dashboard bug above with its own unresolved scroll/coordinate handling;
  building this fix's placement math on top of that would be building on sand. Antonio's actual
  report and its live reproduction were both a region capture (`captureRegion`), not whole-page.
...
- Email-iframe compositing math (intersection, placement, the "did it change" check):
  `npx vitest run tests/unit/capture-email-frame-composite.test.ts`.
- Region capture of an Inbox email (live, browser): open a richer HTML email (not a plain-text
  one — Gmail's own bounce/system notices are a reliable repro), Capture → Select an area over the
  body, confirm the result actually shows the email's content (not blank). Also try a SELECTION
  THAT ONLY PARTIALLY covers the email body — the fix must still composite the full email content
  clipped to what was selected, not just work for a full-email selection. Also try a thread with
  "Expand all" — a capture spanning multiple open emails must composite every one of them.
