...
_Last verified against code: 2026-09-06 — Claude (**The portal-chat destination picker can now send to a company itself, not only to one named person on it.** Antonio, right after the company-name search fix let him find a real two-person company for the first time: "it gives only the member for company but not the company itself to send the message... I want to have all options. the single member as in the screenshot and the company." A multi-member company now offers itself as its own candidate — kind `company_wide`, `contactId: null` — alongside the existing per-person ones, computed by a new pure `computeWholeCompanyCandidates` (one lookup per search, scoped to `account_contacts`, counting only ELIGIBLE — invited — linked contacts; a single-eligible-member company gets no separate candidate, since addressing "the company" and addressing "that one person" would be identical). This mirrors an already-shipped, unrelated feature rather than inventing a new rule: the main Portal Chats composer's own "Whole company" addressing choice (`portal_messages.addressed_to_company`, 2026-09-05) uses the exact same account-only send shape and the exact same multi-member gate (`audienceTotal > 1`) — verified fresh in the composer's own code before building this, not assumed from memory (the independent-verifier caught the first attempt at this same claim as unconfirmed). Genuinely different from the per-person send, not just a label: `notifyClientOfAdminMessage`'s account-only branch notifies EVERY eligible contact at the company, not one, so the confirm screen's copy changes to say so honestly instead of reusing the "one person notified" line. See Gotchas for the exact send-route branching (contact_id becomes optional) and what stayed deliberately out of scope.)_
...
- **`contact_id` is now genuinely OPTIONAL on the portal-chat send route** — the one place in this whole feature where that's true. Every validation step, the destination record's own `id` field (falls back to the account id when there's no contact), the re-check right before sending, and the actual request to the shared send route all branch on whether a contactId was given. Getting this branching wrong in either direction is a real client-facing risk: sending WITH a contactId always means "one specific attributed person" (`sender_context: 'company'`); sending WITHOUT one always means "the account itself, addressed to nobody" (`addressed_to_company: true`, no `sender_context` at all) — the two shapes must never blend, mirroring the main Portal Chats composer's own two distinct request shapes for this exact distinction, not something invented for this feature.
...
- Whole-company send (live, sandbox): search a fixture company with TWO eligible contacts, confirm
  ...
  `portal_messages` for the new row has `contact_id IS NULL` and `addressed_to_company = true`. A
  fixture with only ONE eligible contact must NOT show the whole-company candidate at all.
