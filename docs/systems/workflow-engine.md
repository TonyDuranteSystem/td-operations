...
_Last verified against code: 2026-06-17 — Claude (Company Formation Workspace v2 touched the `formation_progress` task-workflow: the `formation.confirm_ein_received` handler (`lib/tasks/workflow-handlers/formation-confirm-ein-received.ts`) now advances **SS-4 Signed → EIN Received** (was EIN Submitted → Post-Formation + Banking); the catalog row's `actions` are remapped to the 7 v2 stages (separate "Name Check" action dropped; `verify_data_and_name` now targets Filed with State) in migration `20260617-formation-workspace-v2.sql`. `mark_complete` (at EIN Received) still spawns RA Renewal + Annual Report SDs. See formation.md/flows.md.)_
