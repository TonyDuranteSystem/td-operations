...
| `scripts/mac-mini/code-task-runner.mjs` | Mac Mini launchd daemon: claims `recipient='code_runner'` rows, posts "🔧 picked it up" + 5/10-min heartbeats, runs headless `claude --print`, auto-pushes new commits to production, posts result to the Slack thread |
