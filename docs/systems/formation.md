# Company Formation
_Last verified against code: 2026-06-16 — Claude (added `setServiceDeliveryShipping(sdId, {courier, trackingNumber})` — the single authorized write path for the ITIN client shipping-tracking columns (`shipping_courier` / `shipping_tracking_number` / `shipping_submitted_at`, migration `20260616-2300`), since service_deliveries is a protected table. Called by the portal shipping API; stamps `shipping_submitted_at=now()`. See flows.md/portal.md for the full feature.)_
