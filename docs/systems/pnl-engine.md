...
_Last verified against code: 2026-07-02 (smart P&L categorization Phase 1) — Claude (`computePnlTotals` in `lib/pnl-generator.ts` gained three ADDITIVE fields — `foldedUncategorizedCount` / `foldedUncategorizedIncome` / `foldedUncategorizedExpense` — exposing what the `defaultUncategorizedBySign` policy silently folds into income/expenses, so a surface can warn instead of pretending completeness (B&P $594k incident). All pre-existing fields byte-identical; portal gate 6 untouched. `lib/tax/financials-excel.ts` BS section now prints a red "⚠ Unclassified cash movement — categorize to balance" reconciling line (= `uncategorizedTotal`) before the CHECK row, naming the exact assets-vs-equity gap uncategorized rows cause. Tests in `tests/unit/pnl-totals.test.ts` incl. the BS-gap invariant.)_
