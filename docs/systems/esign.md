...
_Last verified against code: 2026-06-27d — Claude (**"Create e-sign document" entry point from the client pages.** Both the CRM **account** detail (`app/(dashboard)/accounts/[id]/page.tsx`) and **contact** detail (`app/(dashboard)/contacts/[id]/page.tsx`) top action rows now have a **Create e-sign document** link → `/tools/esign/new?account=<id>&contact=<contactId>` (shown for all staff, not admin-gated). The editor page (`new/page.tsx`) reads those params server-side, fetches the account + contact, and passes `initialAccount` / `initialSigner` props to `EsignEditor`, which pre-fills the filing account and seeds signer 1 as that CRM client. Plain `<EsignEditor />` with no props is unchanged. Sandbox; ships on Antonio's ok.)_
