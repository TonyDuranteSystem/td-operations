...
VALUES
  ...
  ('Company Formation', 2, 'Wizard Submitted',
   ...
   '{"description": "Client submitted the formation wizard. Review the LLC name choices and owner details, check name availability on the Secretary of State site, and confirm the chosen name with the client.", "components": [{"type": "data_viewer", "label": "Formation Wizard Submission"}, {"type": "external_link", "label": "Check Name Availability on Secretary of State"}, {"type": "decision_requests"}, {"type": "info_panel"}, {"type": "chat"}, {"type": "action_buttons", "actions": [{"key": "advance_next", "label": "Name Confirmed by Client — File with State", "target": "Filed with State"}]}]}'::jsonb),

  ('Company Formation', 3, 'Filed with State',
   (SELECT id FROM catalog_entries WHERE catalog_id='services' AND slug='company_formation'),
   'Filing with the state', 'Registrazione presso lo stato', 'Landmark', true, true, false,
   '{"description": "Name filed on the Secretary of State site. Waiting for the state to confirm (~1 week). When the Articles arrive, advance. If the name was rejected, choose a new name and refile.", "components": [{"type": "info_panel"}, {"type": "external_link", "label": "File on the Secretary of State site"}, {"type": "document_upload", "label": "Upload filing receipt", "autoAdvance": false}, {"type": "document_upload", "label": "Upload Articles of Organization", "autoAdvance": false}, {"type": "decision_requests"}, {"type": "chat"}, {"type": "action_buttons", "actions": [{"key": "advance_next", "label": "Articles Received — Company Created", "target": "Articles Received"}, {"key": "advance_next", "label": "Name Rejected — Choose New Name", "target": "Filed with State"}]}]}'::jsonb),
