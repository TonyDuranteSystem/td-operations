-- Team Workspace S4 — scoped notifications for client conversations.
--
-- Today discussion activity is deliberately SILENT (the "noisy 48" fix): the
-- badge counts DM-unread + mentions only, and channel/discussion chatter never
-- rings. We want a conversation you're PART OF to ring + dot — without
-- resurrecting the noise.
--
-- Participant model (review-endorsed, no new table, no hardcoded team size):
-- you are a participant of a thread iff you have an `internal_thread_reads` row
-- for it — which is created the moment you OPEN it (GET advances the pointer),
-- POST in it, or are seeded as a share recipient. The RPC already LEFT JOINs
-- that table, so `is_participant` is just "the join row exists". A conversation
-- you've never touched contributes nothing to your dot.
--
-- Return-type change → DROP then CREATE. Definition = the S3 grouping function
-- + one projected boolean `is_participant`.
