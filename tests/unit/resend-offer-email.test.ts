...
describe('resendOfferEmail — existing portal user, no valid welcome token (regenerate)', () => {
  it('rotates auth password, issues fresh welcome token, sends portal_notification', async () => {
    ...
    updateUserById.mockResolvedValue({ data: { user: { id: 'auth-1' } }, error: null })
    ;(createWelcomeToken as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({
      token: 'welcome-regen-1',
      welcomeUrl: 'https://app.tonydurante.us/welcome/welcome-regen-1',
    })
    ...
    expect(result.welcomeUrl).toBe('https://app.tonydurante.us/welcome/welcome-regen-1')
    expect(result.gmailMessageId).toBe('gm-1')

    expect(autoCreatePortalUser).not.toHaveBeenCalled()
    expect(updateUserById).toHaveBeenCalledTimes(1)
    expect(updateUserById).toHaveBeenCalledWith('auth-1', expect.objectContaining({ password: expect.any(String) }))
    ...
    expect(logAction).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        details: expect.objectContaining({
          ...
          welcome_url: 'https://app.tonydurante.us/welcome/welcome-regen-1',
        }),
      })
    )
  })

  it('reuses an existing non-expired welcome token without rotating the password', async () => {
  ...
  it('does not block the send when the password rotation fails', async () => {
    setSupabaseStubs(baseOffer)
    ;(findAuthUserByEmail as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 'auth-1', email: baseOffer.client_email,
    })
    ;(findWelcomeTokenBySource as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(null)
    updateUserById.mockResolvedValue({ data: null, error: { message: 'auth admin down' } })
...
describe('resendOfferEmail — status independence', () => {
  for (const status of ['sent', 'viewed', 'signed', 'completed', 'expired']) {
    it(`works on '${status}' offers`, async () => {
      ...
      ;(findAuthUserByEmail as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 'auth-1', email: baseOffer.client_email })
      ;(findWelcomeTokenBySource as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({
        token: 't', welcomeUrl: 'x', expires_at: new Date(Date.now() + 86400000).toISOString(),
      })
