...
export type AppSettingKey =
  ...
  | "email_content_backfill_enabled" // boolean — when true, the off-hours cron (/api/cron/email-content-backfill) automatically downloads every email's body + attachments into our own store (Own-Inbox, dev_task 01800da8). Default OFF (dormant). Turned on from Dev Tools → Email backup; consumed by lib/email-store/auto-backfill.ts.
