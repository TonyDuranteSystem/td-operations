...
export function buildPhantomFileNudge(): string {
  return [
    ...
    "There are exactly TWO tools that produce a file (reach either with `use_tool` if it",
    "is not in your direct list):",
    "  - `pdf_create` — a letter, notice or any prose document. Takes the finished text.",
    "  - `spreadsheet_create` — a table, list, tracker, reconciliation, or a corrected",
    "    copy of a spreadsheet someone uploaded. Takes the rows themselves.",
    "Each returns a real download link.",
    "",
    "So: call the right one NOW with the full content you drafted, and then answer. If you",
