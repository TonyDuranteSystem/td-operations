...
- Day-to-day with portal clients: portal_chat_send (staff -> client via portal chat)
  ✅ portal_chat_send automatically sends the client an email notification (bilingual EN/IT, links to portal chat). Throttled to 1 email per conversation per 2 hours — no manual Gmail follow-up needed for routine messages.
...
- PAYMENT PLANS (shipped 2026-08-11): a setup fee can be sold in PARTS (offers.payment_plan JSONB). Signing bills ONLY part 1; later parts are raised from the account page's "Partial Payment Schedule" box (never by hand-crafting an invoice — the part carries lineage + its own dedup key). Client-facing wording is ALWAYS "Partial Payment", never "installment"/"rata", and plan copy is ALWAYS English regardless of offer language. A plan MAY be combined with a referrer or a managed partner on the same offer (revised 2026-08-14, superseding the earlier authoring-time refusal) — their commission/payout is deferred and released via one manual "Release commission" action on the account page once the ENTIRE plan is paid in real cash, never per-part, never automatically. There is no authoring UI yet: plans are seeded by Antonio/Claude via offer_update, not sellable by staff unaided.
...
- formation_form_*: LLC formation data collection forms for new clients (4 tools). Workflow: after Whop payment → formation_form_create(lead_id, entity_type, state) → send URL via gmail_send → client fills form → formation_form_review(token) → apply changes to CRM. Entity type (SMLLC/MMLLC) and state decided during call (default: SMLLC + NM). Formation pipeline (v2): 8 stages (Payment Confirmed → Wizard Submitted → Filed with State → Articles Received → SS-4 Prepared → SS-4 Signed → SS-4 Sent to IRS → EIN Received). Stage 7 "SS-4 Sent to IRS" is the waiting stage after the fax receipt is uploaded; staff advance to EIN Received manually when the CP 575 arrives. RULE: the CRM account is created ONLY when the Articles arrive (stage 4, Articles Received), where compliance dates + welcome package also fire. The /flows/[sd_id] workspace drives each stage. **Post-payment automation**: activate-service auto-executes ALL steps immediately (supervised mode removed — caused silent failures). Invoices are created at contract signing (contact-only, unpaid) and marked Paid when payment arrives. formation_confirm(activation_id) is for manual recovery only. Works for ALL contract types (formation, onboarding, tax_return, itin, closure).
...
- itin_form_*: ITIN application data collection forms (5 tools). Workflow: itin_form_create(account_id or lead_id) → itin_form_send(token) sends bilingual email with form link → client fills 3-step form (personal info, foreign address, review) → itin_form_review(token, apply_changes) saves data to Drive → itin_prepare_documents(token) generates W-7 + 1040-NR + Schedule OI PDFs and registers them as portal-visible documents (also uploaded to the Drive ITIN folder). The ITIN is then MANAGED FROM THE /flows/[sd] WORKSPACE: staff review the generated docs there (clickable "View") and click "Documents Reviewed — Send to Client" (Document Preparation → Client Signing), which auto-posts the client a bilingual (EN/IT) portal CHAT message telling them the docs are ready in the portal. The client is NEVER emailed the PDFs — the portal is the delivery mechanism (itin_prepare_documents send_email is DEPRECATED; do NOT use it). The staff "Review ITIN documents" notification appears in Portal Chats → What's New as an "Open in Workspace" pointer (no action buttons). Forms are English only. Client must print 2 copies of each document, sign in wet ink, print 2 color passport copies, and mail everything to ${TD_OFFICE.company}, ${TD_OFFICE.street}, ${TD_CITY_STATE_ZIP}. Passport is NOT uploaded digitally — client mails physical copies.
- member_info_form_create: Legacy MMLLC member info collection form (1 tool). Used for MMLLC accounts that existed BEFORE the new member info capture flow — sends primary member a form to submit full details for all LLC members (individual: name/email/phone/address, company: company name/EIN/address/representative). Idempotent — returns existing pending request if one exists. Pre-populates from existing members table. On submit, auto-applies to members table (delete + insert) with no review step. Ownership % is editable by the client. Portal users receive the in-portal viewer URL (portal.tonydurante.us/portal/form/…); others get the public URL (app.tonydurante.us/member-info/…). **MMLLC EIN flow integration**: record-ein-received auto-creates this request and sends a portal message to the primary contact when EIN is recorded on an MMLLC account. Table: member_info_requests.
...
- agent_msg_send / agent_inbox_list / agent_inbox_reply: Hermes ↔ Claude bridge (Phase 1 — research/discussion rail only). Hermes drops research requests via agent_msg_send; a 5-min cron worker (also fired directly when each message lands) invokes claude-sonnet-4-6 with READ-ONLY tools and writes findings back to the same row. Hermes reads replies via agent_inbox_list(filter='my_replies') and reports to Antonio on Telegram. agent_msg_send takes an optional thread_id (WP2) — pass the id from thread_create to give the worker the thread's prior turns as context and a tool surface filtered by the thread type; omit it for a one-shot message. **MANDATORY: agent_msg_send requires Antonio's explicit prior approval (same rule as gmail_send — show the full draft, wait for OK, never send on first turn).** Mutations / sends / code changes are NOT executed by the worker.
- approval_list / approval_decide / approval_claim / approval_execute / approval_complete: DORMANT as of 2026-07-10 (Antonio switched the worker action rail OFF; env WORKER_ACTIONS_ENABLED is unset/off by default). No worker or helper queues actions anymore, so no new pending proposals are created — these tools operate only on the existing (empty) queue and are kept in place for reversibility. Do NOT rely on the rail being live. Original behavior, for reference:
- approval_list: Hermes ↔ Claude bridge Phase 2 (action-authorization rail). The worker's propose_action tool queues a proposed action (send_email, create/update CRM records, advance stage, Drive move/upload, log conversation, save memory) into approval_queue as a 'pending' row that does NOT run. approval_list(status='pending', batch_id?) reads those proposals (including the 6-digit confirmation_code) so a human can see what's awaiting approval; the optional batch_id filter (Phase D) restricts to one group of proposals minted together by batch_propose. approval_list is read-only.
- approval_decide: Hermes ↔ Claude bridge Phase 2 (Slice 2 — DECISION + EXECUTION). approval_decide(id, decision='approve'|'reject', confirmation_code?, note?) decides a PENDING proposal. **MANDATORY: before decision='approve', show Antonio the exact tool_name + params + recipient/cascade/external flags (read the row via approval_list) and wait for his explicit OK — same discipline as gmail_send. Never auto-approve.** **approve REQUIRES the 6-digit confirmation_code shown on the proposal card (WP1) — a missing/wrong code is rejected and the action does NOT run.** approve flips pending→approved (it does NOT instant-fire the server anymore — WP3); the Mac Mini claims and runs it, with the server executor cron as automatic backup. reject (no code needed) flips pending→rejected and notifies Hermes. Only pending rows can be decided. Execution is gated by the APPROVAL_RAIL_ENABLED kill switch (proposals still queue when it's off).
- hermes_heartbeat / approval_claim / approval_execute / approval_complete: Hermes ↔ Claude bridge WP1+WP3 (Operating-Agent pull+execute rail). hermes_heartbeat(instance_id) records liveness so /api/cron/hermes-health can detect an offline instance. approval_claim(instance_id) atomically claims the oldest approved + unclaimed proposal in this env lane (approved→executing), re-checks the params integrity hash (mismatch → fails the row, not handed back), and returns the row to execute (id, tool_name, params, rationale, thread_id, confirmation_code) or a 'nothing to claim' message. **approval_execute(id) (WP3) then runs that claimed row server-side via the same tested executeTool path the server executor uses — the Mac Mini does NOT translate or run the 12 in-dashboard tools itself; the server does, finalizes the row (executed/failed + result/error + executed_at + executed_by), and notifies Hermes + the CRM team chat. approval_execute is gated by APPROVAL_RAIL_ENABLED (ONE master switch — the same flag that gates the server executor cron also gates this Mac Mini path).** approval_complete(id, status, result?, error_text?) remains the manual/fallback closer for any genuinely-local action (approval_execute supersedes the "run it yourself then complete" flow for the 12 server-side tools). Idempotent (only a row still 'executing' is finalized). NOTE (WP3): the Mac Mini is PRIMARY (claim→approval_execute); the server executor cron is BACKUP and only runs an approved row when the Mac Mini is offline/stale or a row strands past the grace window (serverShouldBackstop). The atomic claim still guarantees no double-execution if both ever target the same row.
- thread_create: Hermes ↔ Claude bridge WP2 (thread wiring). thread_create(type, title?, account_id?, contact_id?) opens a new conversation thread and returns { thread_id, type, title }. Hermes calls it before the first agent_msg_send of an investigation, then tags every agent_msg_send in that conversation with the returned thread_id. type ∈ (investigation, action_request, bug_report, client_audit, internal_ops) and narrows the worker's tool surface + shapes its output (e.g. client_audit = CRM read tools only; internal_ops = no client data; action_request must end with a proposal). account_id/contact_id are recorded on the thread (accounts_affected) so it's searchable later via thread_search. thread_create opens the thread only — it runs no action.
- thread_search: Hermes ↔ Claude bridge Phase C (thread intelligence). thread_search(query, type?, tags?, limit?) searches the bridge's durable thread memory (thread_summaries) so a past investigation can be referenced instead of re-derived ("remember the tax-return mismatch thread?"). Free-text query matches title/type/tags/affected-account-ids/outcome/summary; optional type restricts to one thread_type (investigation, action_request, bug_report, client_audit, internal_ops) and tags requires all listed. Newest first, READ-ONLY.
