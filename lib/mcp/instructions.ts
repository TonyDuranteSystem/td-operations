...
- PAYMENT PLANS (shipped 2026-08-11): a setup fee can be sold in PARTS (offers.payment_plan JSONB). Signing bills ONLY part 1; later parts are raised from the account page's "Partial Payment Schedule" box (never by hand-crafting an invoice — the part carries lineage + its own dedup key). Client-facing wording is ALWAYS "Partial Payment", never "installment"/"rata", and plan copy is ALWAYS English regardless of offer language. A plan CANNOT be combined with a referrer or partner on the same offer (offer_create/offer_update refuse it); pre-existing plan+referrer deals suppress the automatic commission and raise a hand-settlement review card instead — commission is settled BY HAND as each part is paid. There is no authoring UI yet: plans are seeded by Antonio/Claude via offer_update, not sellable by staff unaided.
