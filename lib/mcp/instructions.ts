...
- approval_decide: Hermes ↔ Claude bridge Phase 2 (Slice 2 — DECISION + EXECUTION). approval_decide(id, decision='approve'|'reject', confirmation_code?, note?) decides a PENDING proposal. **MANDATORY: before decision='approve', show Antonio the exact tool_name + params + recipient/cascade/external flags (read the row via approval_list) and wait for his explicit OK — same discipline as gmail_send. Never auto-approve.** **approve REQUIRES the 6-digit confirmation_code shown on the proposal card (WP1) — a missing/wrong code is rejected and the action does NOT run.** approve flips pending→approved and fires the executor (/api/cron/approval-executor), which runs the REAL action and writes the outcome back to the row + an agent_messages callback to Hermes; reject (no code needed) flips pending→rejected and notifies Hermes. Only pending rows can be decided. Execution is gated by the APPROVAL_RAIL_ENABLED kill switch (proposals still queue when it's off).
- hermes_heartbeat / approval_claim / approval_complete: Hermes ↔ Claude bridge WP1 (Operating-Agent pull rail). hermes_heartbeat(instance_id) records liveness so /api/cron/hermes-health can detect an offline instance. approval_claim(instance_id) atomically claims the oldest approved + unclaimed proposal in this env lane (approved→executing), re-checks the params integrity hash (mismatch → fails the row, not handed back), and returns the row to execute (id, tool_name, params, rationale, thread_id, confirmation_code) or a 'nothing to claim' message. After running the action the instance MUST call approval_complete(id, status='executed'|'failed', result?, error_text?), which finalizes the row (status + result/error + executed_at + executed_by) and notifies Hermes + the CRM team chat. approval_complete is idempotent (only a row still 'executing' is finalized). NOTE: the server executor (approval-executor cron) and the pull rail both target approved rows; the atomic claim prevents double-execution, but only one runs each row — cut over by toggling APPROVAL_RAIL_ENABLED.
