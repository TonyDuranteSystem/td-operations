...
- banking_form_*: Payset EUR banking application data collection forms for existing clients (3 tools). Workflow: banking_form_create(account_id) → send URL via gmail_send → client fills form (personal info, business info, proof of address, bank statement) → banking_form_review(token) → apply changes: advances Banking Fintech service delivery + creates task for live Payset session. Payset has NO API — onboarding is manual (live session with OTP codes). Form collects Fase 1 data only.
