...
- offer_*: Service proposals — create, list, get, update, send (5 tools). All JSONB fields use English names (services, cost_summary, issues, strategy, etc.). Workflow: create (draft) → review → offer_send (creates Gmail draft) → client views → signs → pays. **Contract types**: 'msa' (default, new clients/formation), 'service' (existing clients becoming annual — same MSA+SOW, no formation language), 'tax_return' (tax filing only, lightweight 8-section agreement). **CRITICAL**: All contract content (services, cost_summary, recurring_costs) MUST be in English regardless of offer language. For existing clients use account_id (not lead_id).
