...
- approval_list: Hermes ↔ Claude bridge Phase 2 (action-authorization rail). The worker's propose_action tool queues a proposed action (send_email, create/update CRM records, advance stage, Drive move/upload, log conversation, save memory) into approval_queue as a 'pending' row that does NOT run. approval_list(status='pending') reads those proposals so a human can see what's awaiting approval. approval_list is read-only.
