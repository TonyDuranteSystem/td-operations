...
export function registerAgentThreadTools(server: McpServer) {
  ...
  server.tool(
    ...
    [
      ...
      "  investigation   — general research; full read tools + propose_action.",
      "  action_request  — Antonio wants something done; worker MUST end with ≥1 propose_action.",
      "  bug_report      — codebase + CRM read tools; output as Summary / Repro / Root cause (file:line) / Fix.",
      "  client_audit    — CRM read tools ONLY (no code, no propose); client-facing review.",
