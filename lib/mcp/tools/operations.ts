  ...
  server.tool(
    ...
    async ({ delivery_id, target_stage, skip_tasks, notes }) => {
      try {
        ...
        if (!result.success) {
          return { content: [{ type: "text" as const, text: `⚠️ ${result.error}` }] }
        }

        const statusIcon = result.failed_tasks.length > 0 ? "⚠️" : "✅"
        const lines = [
          `${statusIcon} Advanced to: **${result.to_stage}** (stage ${result.to_order}/${result.total_stages})`,
          `🔄 From: ${result.from_stage} → ${result.to_stage}`,
        ]
        if (result.sla_days) lines.push(`⏱️ SLA: ${result.sla_days} days`)
        if (result.requires_approval) lines.push(`🔒 This stage requires approval before advancing`)
        if (result.created_tasks.length > 0) {
          lines.push(`\n📝 Auto-created ${result.created_tasks.length} tasks:`)
          for (const t of result.created_tasks) lines.push(`  ✅ ${t}`)
        }
        if (result.failed_tasks.length > 0) {
          lines.push(`\n❌ Failed to create ${result.failed_tasks.length} tasks:`)
          for (const t of result.failed_tasks) lines.push(`  ❌ ${t.title} — ${t.error}`)
        }
        if (result.is_completed) lines.push(`\n🎉 Service delivery marked as COMPLETED`)
        if (result.auto_triggers.length > 0) {
          lines.push(`\n🔧 Auto-triggers:`)
          for (const trigger of result.auto_triggers) lines.push(`  → ${trigger}`)
        }
