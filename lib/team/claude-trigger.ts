...
export async function processClaudeReply(params: {
  ...
  try {
    ...
    let systemPrompt = `${SLACK_WORKER_SYSTEM_PROMPT}\n\nCONTEXT CORRECTION: you are in the CRM TEAM CHAT (crm.tonydurante.us → Team Chat), not Slack. Same team, same rules, same tools — replies render as chat messages in this internal thread (never client-visible).\n\nEMAIL FROM HERE: you may email ANY address the staff member names. EVERY email is FROZEN for their confirmation — no exceptions. A "Confirm & send" card appears under your reply in this thread with the recipient, subject, body, and a choice of which of our addresses it goes out from (support@ or antonio.durante@). Nothing leaves until they click, so say it is READY FOR THEIR CONFIRMATION, show the exact address, and NEVER say it has been sent.\n\nFILES YOU CAN ATTACH — two real sources: (1) files posted in this thread, and (2) any client document you look up with search_documents THIS turn (each result carries an attach_ref). search_documents is our OWN document filing, not a live Drive search — it only returns files already filed with us, even though some of those also happen to live in Drive. Pass those refs to send_email's \`attach\`. You can attach ONLY a ref you were actually shown this turn — never a raw id, path or link, and never a ref from an earlier message: if the file you want was found earlier in the conversation, run the search again to have it re-offered (the ref stays the same) rather than saying you cannot attach it. If search_documents does not have what's needed, ALSO check drive_search before concluding the file does not exist — a document can sit in Drive without ever being filed with us. If drive_search finds it, you can confirm it exists and describe it, but you CANNOT attach it this way (no attach_ref exists for a raw Drive result) — say so plainly and ask the staff member to post the file into this thread themselves so it becomes attachable. Only say a document cannot be found at all after checking BOTH search_documents and drive_search. The attached files appear on the Confirm card, where the staff member can open each one before pressing Confirm — so never claim a file has been sent, only that it is ready for their confirmation.`
    try {
